...
8. [FRT/VRT 多路选择器：浮点与向量统一接口](#8-frt-vrt-多路选择器)
...
- **每周期处理 4 条指令**（inst0 ~ inst3，顺序最新的为 inst0）
...
浮点指令有额外复杂性：浮点目的寄存器（f0~f31）是独立的重命名域，并且还有扩展寄存器（ereg，用于 fclass/fmv.x.* 等将浮点结果写整数的指令）。
...
RTU 提前分配好这些号，IR 阶段直接使用，无需当拍申请，这是避免关键路径延迟的重要设计。
...
**为什么用时钟门控？** IR 寄存器共有 `4×178 + 17 = 729` 个触发器。每个周期不一定都有新指令流入（例如 stall 周期），此时关闭时钟可以节省大量动态功耗。门控条件确保：有新指令传入时打开（流入），IR 已有有效指令时也要保持打开（维持当前值直至 stall 解除）。
...
4 个实例完全并行工作，每周期同时对 4 条指令完成 IR 级再译码。
...
assign ir_inst0_dst_ereg = rtu_idu_alloc_ereg0;  // 扩展寄存器（fclass 等）
```
...
**MUX 的选择逻辑**：以 `IR_SRCV0_VLD` 为选择信号——如果向量源 0 有效，说明是向量指令，用 vrt 的结果；否则是浮点指令，用 frt 的结果（此时 `IR_SRCF0_VLD` 已经保证有效）。两者不会同时为 1（一条指令要么是向量要么是浮点标量）。
...
重命名表（ir_rt）保存的是**已提交**或**已经过之前周期重命名**的映射关系。当同一周期内 4 条指令同时进入 IR 阶段时，发生以下场景：
...
inst1/2/3 的组装逻辑与 inst0 完全对称（行 2166~2440），只是对应的信号下标不同。4 条指令并行组装，同一个时钟周期内完成。
...
`IS_LCH_PREG` 是"launch physical register"的缩写，表示该指令在 IS 阶段需要提前（早于正常发射）读取物理寄存器文件：
...
### 13.2 为什么只有 ALU 和 SPECIAL 需要提前读

ALU/SPECIAL 指令的延迟很短（1~2 周期），当它们发射出去执行时，等待它们结果的后续指令可能很快就要读寄存器。如果在 IS 阶段就提前把物理寄存器号锁存在发射队列入口，发射时就不用再去查询，可以节省关键路径上的一拍延迟。

LSU 和向量指令延迟长，不需要这个优化——等它们发射时，读寄存器文件的时序余量更充裕。
...
| lch_preg 预计算 | 2050~2066 | ir_inst*_lch_preg | RF launch 提前优化 |
