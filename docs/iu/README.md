...
> IU 共 14 个 RTL 文件，约 1.2 万行，位于 `C910_RTL_FACTORY/gen_rtl/iu/rtl/`。
...
| [04_mult.md](04_mult.md) | 乘法/乘累加单元：3 级乘法器内核、EX1~EX4 可见结果路径、MLA 前递链 | ct_iu_mult.v + multiplier_65x65_3_stage.v |
...
- **上游**：IDU 的发射与读寄存器见 `docs/idu/`（特别是 13_is_aiq / 14_is_biq 发射，
  19_rf_dp 读数，20_rf_fwd 前递网络——IU 的 RBUS 正是前递网络的数据源）。
- **分支闭环**：IFU 侧的预测机制见 `docs/ifu/`（03_btb / 04_bht / 05_ras）和
  `docs/branch_prediction.md`；本目录的 02_bju / 03_bju_pcfifo 讲"检查与纠错"的另一半。
- **下游**：完成/退休见 RTU 文档 `docs/rtu/`。
