...
消费者需要分支指令的完整 PC 与预测信息：

1. **BJU 执行时**：要对比"当时预测的方向/目标"和"现在算出的真实值"；
2. **pipe0 上的链接微操作**：jal/jalr 写 rd=PC+4 被 IDU 拆分后，算链接地址的
   微操作要知道原指令 PC；
3. **RTU 退休时**：要恢复精确退休 PC（顺序遍历：非跳转指令 PC 递增，跳转指令
   PC 从 PCFIFO 弹出值跳变）。

PCFIFO 就是为这三类消费者建立的 **32 项环形缓冲**：IFU 取指时按程序序创建，
BJU 执行时随机读+回写，RTU 退休时按程序序弹出。它本质上是
"**分支指令的影子 ROB**"。
...
背景：IDU 把带链接的跳转拆成两个微操作——pipe0 上的"算 PC+4 写 rd"和 pipe2
上的"跳转检查"。**两个微操作各拿一个 pid，各自从 PCFIFO 读自己需要的 PC**：
...
entry0 单项 + entry1 双项。IFU 保证不会出现 2+2。
...
独热指针移位即步进（`{ptr[30:0],ptr[31]}`），用作读/写选择时**无需译码**；
...
`create5_ptr` 是 create0 前方第 5 个位置——它还被占着就说明**空闲项少于 6**，
即告满。留 6 项余量的原因：IFU 创建请求在途、暂存一拍、单拍最多 3 项写入，
反压信号传回 IFU 也有延迟，必须为"已在路上的创建"预留空间。
...
- 满阈值留 6 余量意味着有效容量约 26 项常驻分支；
- coremark 这类每 5~6 条指令一个分支的负载，ROB 满 96 项时在飞分支约 16~20 个，
  PCFIFO 容量基本匹配 ROB 规模——这是"32"这个数字的来历。
