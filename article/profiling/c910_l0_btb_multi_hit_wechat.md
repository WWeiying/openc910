# C910 前端里的隐蔽问题：L0-BTB 为什么会拼出一个不存在的跳转目标
...
![图 1：L0-BTB 对多个命中项的 Target 进行按位 OR](屏幕截图%202026-08-15%20165530.png)

只要任意命中项的 Counter 允许预测，L0-BTB 就会接受这次重定向。

![图 2：Counter 汇总与早期重定向条件](屏幕截图%202026-08-15%20165651.png)

合成的 Target 随后被直接送入 PCGEN。

![图 3：L0-BTB 输出目标的生成逻辑](屏幕截图%202026-08-15%20165806.png)

下一拍，IP 级发现预测错误，又把 PC 从 `0x1b7c` 纠正回真实目标 `0x1a70`。

![图 4：错误早转向以及随后的 IP 级纠正](屏幕截图%202026-08-15%20170426.png)
