...
![图 1：Hot Chips 2024 的 Snapdragon X Elite 纪念章](qualcomm_oryon_hot_chips_2024_figures/01_figure.jpg)
...
![图 2：Oryon 分支预测器的容量预算](qualcomm_oryon_hot_chips_2024_figures/02_figure.jpg)
...
![图 3：Oryon 前端与 8-wide 管线](qualcomm_oryon_hot_chips_2024_figures/03_figure.jpg)
...
![图 4：Oryon 标量与向量执行布局](qualcomm_oryon_hot_chips_2024_figures/04_figure.jpg)

Qualcomm 选择分布式 Scheduler。小队列更容易从 Ready 操作中选出较老者，也利于高速时序；代价是操作只能等待指定端口，可能出现某队列拥堵、相邻端口闲置。

![图 5：Load/Store 调度资源](qualcomm_oryon_hot_chips_2024_figures/05_figure.jpg)
...
![图 6：Load 调度容量微基准](qualcomm_oryon_hot_chips_2024_figures/06_figure.png)

![图 7：Store 相关容量测试](qualcomm_oryon_hot_chips_2024_figures/07_figure.png)
...
![图 8：Oryon L1D 设计](qualcomm_oryon_hot_chips_2024_figures/08_figure.jpg)
...
![图 9：Oryon TLB 与地址翻译延迟](qualcomm_oryon_hot_chips_2024_figures/09_figure.png)
...
![图 10：Hot Chips 展示的 Snapdragon X Elite](qualcomm_oryon_hot_chips_2024_figures/10_figure.jpg)
...
![图 11：Oryon Cache/内存延迟](qualcomm_oryon_hot_chips_2024_figures/11_figure.png)

激进预取让单线程 DRAM 带宽接近 100 GB/s，其他测试核心约 30 GB/s。独立内存带宽结果也显示，Oryon 单核明显高于 Meteor Lake Redwood Cove，不过 Intel 的 L1/L2 峰值更高。

![图 12：单线程 Cache 与 DRAM 带宽](qualcomm_oryon_hot_chips_2024_figures/12_figure.png)

L2 之后还有 6 MB System Level Cache（SLC），容量可在 SoC Block 间动态分配。相对 12 MB CPU L2，它对 CPU 的作用较小，更适合 GPU、Display 和其他缺乏大私有 Cache 的模块。

![图 13：Snapdragon X Elite 的 6 MB SLC](qualcomm_oryon_hot_chips_2024_figures/13_figure.png)

Oryon 配置多种标准和自研 Prefetcher。简单线性访问中，预取能够跑得足够远，几乎隐藏 L2 延迟；更复杂模式的收益取决于能否识别。

![图 14：不同访问模式下的可见 Load 延迟](qualcomm_oryon_hot_chips_2024_figures/14_figure.jpg)
