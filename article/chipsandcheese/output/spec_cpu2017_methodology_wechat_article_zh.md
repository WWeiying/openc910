...
![图 1：第一批 SPEC CPU2017 Estimated 总分；3950X 的 Linux 环境未支持 Boost](spec_cpu2017_methodology_figures/01_figure.png)

![图 2：AnandTech 最后一篇 Ryzen 9 9950X 文章中的 SPEC 结果，作为外部参照](spec_cpu2017_methodology_figures/02_figure.png)
...
![图 3：Integer Suite 各子项相对参考系统的速度比](spec_cpu2017_methodology_figures/03_figure.png)

![图 4：Floating-point Suite 子项速度比，Outlier 更明显](spec_cpu2017_methodology_figures/04_figure.png)

PMU 深挖集中于可用的最新 AMD Zen 5 与 Intel Redwood Cove，既代表现代设计，也可缩短一次耗时数小时的运行。

![图 5：Zen 5 与 Redwood Cove 的 Integer 子项 IPC](spec_cpu2017_methodology_figures/05_figure.png)

Integer 中 `548.exchange2`、`525.x264`、`500.perlbench` IPC 很高；`505.mcf`、`520.omnetpp` 很低，两核的相对模式大致一致。

![图 6：FP 子项 IPC；538.imagick 很高，549.fotonik3d 很低](spec_cpu2017_methodology_figures/06_figure.png)
...
![图 7：Zen 5 的 505.mcf 与 520.omnetpp Top-down 分解](spec_cpu2017_methodology_figures/07_figure.png)

![图 8：Redwood Cove 的对应分解](spec_cpu2017_methodology_figures/08_figure.png)
...
![图 9：SPEC Integer 的 Branch Prediction Accuracy/MPKI](spec_cpu2017_methodology_figures/09_figure.png)

![图 10：Branch Density；xalancbmk 分支更多但准确率高，最终 IPC 尚可](spec_cpu2017_methodology_figures/10_figure.png)
...
![图 11：Integer Suite 的 Micro-op Cache Coverage；Zen 5 在 leela、deepsjeng 仍超过 90%](spec_cpu2017_methodology_figures/11_figure.png)
...
![图 12：505.mcf 的 Instruction Cache Miss；L1I Miss 并非主要原因](spec_cpu2017_methodology_figures/12_figure.jpg)

Direct Branch 占绝大多数，Return 与 Indirect Branch 只是少数。

![图 13：505.mcf 的动态 Branch Type 组成](spec_cpu2017_methodology_figures/13_figure.png)

Zen 5 对 Return 与 Changing-target Indirect Branch 不能 Zero-bubble，但其 Indirect Predictor 使用次数竟超过 Retired Indirect Branch 两倍。

![图 14：Zen 5 的 BTB/Indirect Predictor Delay 事件](spec_cpu2017_methodology_figures/14_figure.jpg)
...
![图 15：Redwood Cove 运行 505.mcf 时的数据 Cache 与 DTLB Miss](spec_cpu2017_methodology_figures/15_figure.png)

多数 Miss 被 L2/L3 接住，但 1.97 L3 MPKI 仍因 DRAM 极高延迟贡献大部分 Memory-bound Delay。

![图 16：存在 Pending Cache Miss 时全部执行单元空闲的周期](spec_cpu2017_methodology_figures/16_figure.png)
...
![图 17：Redwood Cove 单核运行 SPEC Integer 的读写 DRAM Bandwidth](spec_cpu2017_methodology_figures/17_figure.png)

FP 大体相似，但 `549.fotonik3d` 与 `503.bwaves` 明显突出。

![图 18：SPEC FP 的读写 DRAM Bandwidth](spec_cpu2017_methodology_figures/18_figure.png)
...
![图 19：Redwood Cove 的 FP Top-down 分解，fotonik3d 明显 Backend Memory-bound](spec_cpu2017_methodology_figures/19_figure.png)

FP Suite 整体比 Integer 更 Core-bound，更多 Slot 退休有用工作，更强调 Execution Latency/Throughput；Zen 5 模式类似，仍有少量 Frontend Latency。

![图 20：Zen 5 的 FP Top-down 分解](spec_cpu2017_methodology_figures/20_figure.png)

`507.cactuBSSN` 是全套中唯一让 Zen 5 Op-cache Hit 低于 90%的项目，Redwood Cove 仅 58.98%。

![图 21：FP Suite 的 Micro-op Source](spec_cpu2017_methodology_figures/21_figure.png)
...
![图 22：Zen 4 Integer Suite 的 Demand Cache Refill](spec_cpu2017_methodology_figures/22_figure.png)
...
![图 23：Zen 4 FP Suite 的 Demand Refill；fotonik3d、roms 的 L3 Miss 是极端例外](spec_cpu2017_methodology_figures/23_figure.png)

FP 项常呈两极：32 MB L3 已覆盖，或连 96 MB 也难彻底覆盖。前者不能受益，后者 Hit 增幅又可能不足以抵消 V-Cache CCD 的降频。

![图 24：7950X3D 两个 CCD 的 SPEC FP IPC 与分数差异](spec_cpu2017_methodology_figures/24_figure.png)
...
![图 25：7950X3D 两个 CCD 的 SPEC Integer 差异](spec_cpu2017_methodology_figures/25_figure.png)
