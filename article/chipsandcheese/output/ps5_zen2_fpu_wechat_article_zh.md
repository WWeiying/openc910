...
![图 1：桌面 Zen 2 与 PS5 风格核心的 FPU Die Photo](ps5_zen2_fpu_figures/01_figure.jpg)
...
![图 2：微基准反推的 FPU 端口能力](ps5_zen2_fpu_figures/02_figure.png)

*图 2：来自指令吞吐与 PMU，不是 AMD 官方框图。*

![图 3：物理寄存器阵列的版图变化](ps5_zen2_fpu_figures/03_figure.jpg)
...
![图 4：PS5 与普通 Zen 2 的 FP Register Capacity](ps5_zen2_fpu_figures/04_figure.png)
...
![图 5：Zen 4 关于寄存器文件端口主导面积的说明](ps5_zen2_fpu_figures/05_figure.jpg)

*图 5：Kai Troester 讨论的是 Zen 4 512-bit Register File，本文把同一物理规律用于解释 PS5，是推断。*

![图 6：Zen 2 Register Read Port 复用示意](ps5_zen2_fpu_figures/06_figure.png)
...
![图 7：完整乱序结构配较少执行端的意义](ps5_zen2_fpu_figures/07_figure.png)
...
![图 8：BC-250 与 Steam Deck 平台差异](ps5_zen2_fpu_figures/08_figure.png)
...
![图 9：两平台 DRAM 带宽](ps5_zen2_fpu_figures/09_figure.png)
...
![图 10：三核 7-Zip 基线](ps5_zen2_fpu_figures/10_figure.png)

*图 10：BC-250 快 4%，更低内存延迟可能是原因；后续差异若在 4% 内视为基本持平。*

libx264 用一个 4K Overwatch 游戏片段转码，Steam Deck 快 14.9%。

![图 11：libx264 三核性能](ps5_zen2_fpu_figures/11_figure.png)

*图 11：结果同时含平台差异，但已明显超过 4% 基线。*

![图 12：libx264 的 FP Pipe Assignment](ps5_zen2_fpu_figures/12_figure.png)
...
![图 13：Zen 2 PPR 的 FP 资源 Stall 事件](ps5_zen2_fpu_figures/13_figure.png)

*图 13：后续用这两类事件区分吞吐与在途寿命。*

![图 14：libx264 的 FP Register/Scheduler Stall](ps5_zen2_fpu_figures/14_figure.png)
...
![图 15：SSIM 性能](ps5_zen2_fpu_figures/15_figure.png)

*图 15：中等 FP 负载可被精简 FPU 吸收。*

![图 16：SSIM 的 Pipe 分配](ps5_zen2_fpu_figures/16_figure.png)

*图 16：FP2 因 Vector Store 得到较多工作，正好利用 PS5 保留的 Store Pipe。*

![图 17：SSIM 的 FP Resource Stall](ps5_zen2_fpu_figures/17_figure.png)
...
![图 18：Y-Cruncher 性能](ps5_zen2_fpu_figures/18_figure.png)

*图 18：三核设置用于降低平台带宽失配，但无法完全消除。*

![图 19：Y-Cruncher 的 Pipe 利用率](ps5_zen2_fpu_figures/19_figure.png)

*图 19：普通四 Pipe 都很忙；PS5 的 FP0/1 更重，FP0 接近 70%。*

![图 20：Y-Cruncher 的 FP Scheduler Full](ps5_zen2_fpu_figures/20_figure.png)
...
![图 21：游戏中的 FP Pipe 利用率](ps5_zen2_fpu_figures/21_figure.png)

*图 21：这是所测游戏的平均值，不能保证所有游戏和瞬时阶段都相同。双数学 Pipe 对 PS5 的目标负载很合理。*

## 从定制 FPU 到 Zen 4c：两种面积策略

![图 22：Cortex-A510 可选 VPU Pipe](ps5_zen2_fpu_figures/22_figure.png)

*图 22：Arm 明确提供可配置 128-bit Pipe；PS5 表明 AMD 也有很强的客户定制能力，只是未公开通用选项。*

![图 23：PS5 游戏的 FP 操作与乱序缓冲](ps5_zen2_fpu_figures/23_figure.png)

*图 23：即使游戏每秒执行数十亿 FP Operation，完整 Queue 仍能吸收短时峰值，精简执行端足够。*

![图 24：FPU 面积的像素测量](ps5_zen2_fpu_figures/24_figure.jpg)

*图 24：缩小约 35%，但四核 Cluster 只缩约 5.8%；比例来自 Die Photo 缩放、标注和像素计数，并非 AMD Floorplan 数字。*

![图 25：Zen 4c 的整核密度路线](ps5_zen2_fpu_figures/25_figure.jpg)
