...
![图 1：Cortex-A55 在 DynamIQ 集群中的定位](arm_cortex_a55_figures/01_cortex_a55_product_timeline.png)
...
![图 2：Cortex-A55 流水线](arm_cortex_a55_figures/02_cortex_a55_overview.png)
...
![图 3：Arm 对 A55 分支预测改进的说明](arm_cortex_a55_figures/03_neural_branch_predictor.png)
...
![图 4：Cortex-A55 的随机方向模式识别](arm_cortex_a55_figures/04_a55_branch_pattern.jpg)

*图 4：Pattern Length 与活跃分支数增加后，低延迟平面逐渐破裂。曲面同时混入可学习历史、权重容量和别名，不能单独证明 Perceptron 的表组织。*

![图 5：Cortex-A53 的同类方向模式识别](arm_cortex_a55_figures/05_a53_branch_pattern.png)
...
![图 6：Geekbench 6 的 Branch MPKI](arm_cortex_a55_figures/06_geekbench_branch_mpki.png)
...
![图 7：Micro-BTB 容量与 Branch Spacing](arm_cortex_a55_figures/07_micro_btb_capacity.png)
...
![图 8：Taken Branch 足迹与延迟](arm_cortex_a55_figures/08_taken_branch_latency.png)
...
![图 9：Predecode L1I 的吞吐边界](arm_cortex_a55_figures/09_l1i_predecode_ecc.png)
...
![图 10：A55、A53 的代码供给与前端空转](arm_cortex_a55_figures/10_instruction_fetch_throughput.png)
...
![图 11：A55 后端 Forwarding Path](arm_cortex_a55_figures/11_backend_forwarding_paths.png)
...
![图 12：Geekbench 6 中 A55 的后端 Interlock](arm_cortex_a55_figures/12_fp_vector_latency.png)
...
![图 13：HDR 热点中的 `fmadd`](arm_cortex_a55_figures/13_hdr_fmadd_code.png)

*图 13：网页正式图注指出，Geekbench 6 HDR 最热函数包含 `fmadd`，A55 改善的四周期 FMA 很可能直接帮助该负载。它是关联证据，不是单指令贡献的严格消融。*

![图 14：A53 的后端 Interlock 对照](arm_cortex_a55_figures/14_geekbench_interlock_stalls.png)
...
![图 15：A55 的 Load/Store Datapath 改进](arm_cortex_a55_figures/15_load_store_throughput.png)
...
![图 16：两条 Load Forwarding Path](arm_cortex_a55_figures/16_load_forwarding_paths.png)
...
![图 17：依赖 Load 之间插入 ALU 的测试](arm_cortex_a55_figures/17_dependent_load_alu_latency.png)
...
![图 18：Cortex-A55 的 Store-to-Load Forwarding](arm_cortex_a55_figures/18_a55_store_forwarding.jpg)
...
![图 19：A55 与 A53 的典型 Forwarding 延迟](arm_cortex_a55_figures/19_store_forwarding_summary.png)

*图 19：非整数平均值说明某些迭代偶尔多一周期，例如最佳情形 A55 更常为四周期、A53 更常为三周期。双发射支持可能增加 LSU 复杂度，但并无 RTL 证据。*

![图 20：Cortex-A53 的 Store-to-Load Forwarding](arm_cortex_a55_figures/20_a53_store_forwarding.png)
...
![图 21：A55 与 A53 的 TLB 结构](arm_cortex_a55_figures/21_tlb_capacity_latency.png)
...
![图 22：VIPT Cache 的索引与物理标签](arm_cortex_a55_figures/22_vipt_cache_indexing.png)
...
![图 23：顺序管线无法覆盖 Cache Miss](arm_cortex_a55_figures/23_memory_stall_window.png)
...
![图 24：A55 私有 L2 带来的延迟台阶](arm_cortex_a55_figures/24_private_l2_design.png)
...
![图 25：DSU L3 的可配置流水级](arm_cortex_a55_figures/25_dsu_l3_latency_options.png)
...
![图 26：A55 与 A53 的 Cache/Memory 延迟](arm_cortex_a55_figures/26_cache_memory_latency.png)
...
![图 27：不同 A55 实现与 A53 的读取带宽](arm_cortex_a55_figures/27_read_bandwidth.png)
...
![图 28：Demand Refill 与 Prefetch 并发](arm_cortex_a55_figures/28_demand_prefetch_parallelism.png)
...
![图 29：Geekbench 6 的读取与写入带宽](arm_cortex_a55_figures/29_write_bandwidth.png)
...
![图 30：Geekbench 6 的 Memory Stall](arm_cortex_a55_figures/30_geekbench_memory_stalls.png)
...
![图 31：Arm 2017 年的 A55/A75 参考增幅](arm_cortex_a55_figures/31_geekbench6_performance.jpg)
...
![图 32：SPEC CPU2026 整数套件估算分数](arm_cortex_a55_figures/32_spec_cpu2026_integer.png)

*图 32：14 项整数工作负载全部由 A55 领先，几何平均相对 A53 提升 35.35%。图题明确为 Estimated Scores；测试版本虽为 SPEC CPU2026，网页未完整给出编译参数和运行统计。*

![图 33：SPEC CPU2026 浮点套件估算分数](arm_cortex_a55_figures/33_spec_cpu2026_fp.png)
...
![图 34：A55 在 SPEC CPU2026 中的 IPC](arm_cortex_a55_figures/34_geekbench_ipc.png)

*图 34：多数项目只有约 0.35～0.9 IPC，少数刚过 1，远低于两宽理论上限。这是顺序核很难持续喂满的直接证据。*

![图 35：Geekbench 6 的 A55、A53 与 A73](arm_cortex_a55_figures/35_a55_a73_performance.png)

*图 35：A55 相对 A53 几乎逐项提升，但两宽乱序 A73 多数仍明显更快。大 Cache 与更好预测可以减少停顿，却无法复制乱序调度跨过依赖/Miss 的能力。*

![图 36：Geekbench 6 总分与小核面积路线](arm_cortex_a55_figures/36_a55_a510_area.png)
