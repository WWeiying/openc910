...
![图 1：一个 8 Byte 原子值横跨两条 64 Byte Cache Line 的示意](x86_split_locks_figures/01_figure.jpg)
...
![图 2：Arrow Lake 普通 Cache Line Ping-pong Latency](x86_split_locks_figures/02_figure.jpg)

Split Lock 把延迟推到约 7 μs，且不同 Core Type 之间差异不大。

![图 3：Arrow Lake Split Lock 的跨核延迟矩阵](x86_split_locks_figures/03_figure.png)

![图 4：Arrow Lake 普通与 Split Lock 延迟对照](x86_split_locks_figures/04_figure.jpg)

干扰只从 L2 Miss 开始，行为接近传统意义上的“锁住所有核心共享层级”：若另一程序持续命中本核 L2 或更快 Cache，可以不受影响。

![图 5：Arrow Lake 在 Split Lock 干扰下的 Memory Latency](x86_split_locks_figures/05_figure.png)

![图 6：Arrow Lake Memory Bandwidth；越过 L2 后大致减半](x86_split_locks_figures/06_figure.png)
...
![图 7：Arrow Lake 的两项 GB6 干扰结果；P-Core 4.7 GHz、E-Core 3.8 GHz、DDR5-4800 JEDEC](x86_split_locks_figures/07_figure.png)

## Zen 5：Split Lock 本身较快，邻居反而更惨

Ryzen 9 9900X 的 Split Lock 约 500 ns，优于 Arrow Lake，仍远慢于普通 Atomic；CCX Boundary 基本不改变延迟。

![图 8：Zen 5 普通跨核延迟](x86_split_locks_figures/08_figure.jpg)

![图 9：Zen 5 Split Lock 延迟](x86_split_locks_figures/09_figure.jpg)

但它会破坏所有 L1D Miss：L2、L3 Latency 与 Bandwidth 约退化 10 倍。

![图 10：Zen 5 Memory Latency，在 L1D 以外出现数量级退化](x86_split_locks_figures/10_figure.png)

![图 11：Zen 5 Memory Bandwidth](x86_split_locks_figures/11_figure.png)

所以连平时不产生太多 L3 Miss 的 Asset Compression 也重挫，因为此时一次 L1D Miss 就已昂贵。

![图 12：Zen 5 两项 GB6 Workload 均严重下降](x86_split_locks_figures/12_figure.png)
...
![图 13：Alder Lake 普通跨核 Latency](x86_split_locks_figures/13_figure.png)

Split Lock 颠倒关系：P-Core 极慢，P↔E 略高于 7 μs、接近 Arrow Lake，E-Core 反而最好。

![图 14：Alder Lake Split Lock Latency](x86_split_locks_figures/14_figure.png)

其 Memory Subsystem 却很能隔离干扰，L3 只轻微下降，DRAM Latency 增量也不大。

![图 15：Alder Lake Memory Latency](x86_split_locks_figures/15_figure.png)

![图 16：Alder Lake Memory Bandwidth](x86_split_locks_figures/16_figure.png)

![图 17：GB6 几乎不受影响；此平台未降频，部分差异来自多核频率变化](x86_split_locks_figures/17_figure.png)

## Zen 2：与 Zen 5 一样，从 L1D Miss 起全面受创

Ryzen 9 3950X 的 Split Lock 延迟高于 Zen 5，仍好于新 Intel；CCX Boundary 同样没有明显影响。

![图 18：Zen 2 普通跨核 Latency](x86_split_locks_figures/18_figure.jpg)

![图 19：Zen 2 Split Lock Latency](x86_split_locks_figures/19_figure.jpg)

L1D 之外 Latency/Bandwidth 约退化 10 倍。

![图 20：Zen 2 Memory Latency 与 Bandwidth 的 Split Lock 对照](x86_split_locks_figures/20_figure.png)

当 Split Lock 与被测负载位于同一 CCX 时，L3 Latency 在 2 MB 后还出现一次额外拐点，Zen 5 同 CCX 没有相同现象。

![图 21：Zen 2 工作集越过 2 MB 后的额外 L3 Latency 变化](x86_split_locks_figures/21_figure.png)

![图 22：Zen 2 的 GB6 Photo Filter 与 Asset Compression 均严重下降](x86_split_locks_figures/22_figure.png)

## Skylake：L2 Hit 安全，越过 L2 才受罚

Core i5-6600K 的 Split Lock 竟优于 Arrow Lake/Alder Lake，略差于 Zen 2，尚未到 μs 级。

![图 23：Skylake 普通与 Split Lock 跨核 Latency](x86_split_locks_figures/23_figure.png)

和 Arrow Lake 类似，L1/L2 Hit 不受影响，L2 Miss 后出现惩罚。

![图 24：Skylake Memory Latency](x86_split_locks_figures/24_figure.png)

![图 25：Skylake Memory Bandwidth](x86_split_locks_figures/25_figure.png)

![图 26：Photo Filter 性能下降 34.24%，Asset Compression 下降 16.2%](x86_split_locks_figures/26_figure.png)

## Piledriver：最老的平台反而最从容

FX-8350 平时跨核延迟高，但 Split Lock 只比普通 Atomic 慢 2～3 倍，是全部平台最好结果。

![图 27：Piledriver 普通与 Split Lock Latency](x86_split_locks_figures/27_figure.png)

Cache Hit 完全不受影响，甚至包括 Shared L3；只有 DRAM Latency 约翻倍、Bandwidth 下降一半以上。

![图 28：Piledriver Memory Latency](x86_split_locks_figures/28_figure.png)

![图 29：Piledriver Memory Bandwidth](x86_split_locks_figures/29_figure.png)

![图 30：两项 GB6 只有较轻损失](x86_split_locks_figures/30_figure.png)
...
![图 31：Goldmont Plus 跨核 Atomic Latency](x86_split_locks_figures/31_figure.png)

和 Arrow Lake E-Core 一样，L2 Hit 不受影响；DRAM Bandwidth 显著下降，Latency 只略增，不过它的 Baseline DRAM Latency 本来就很差。

![图 32：Goldmont Plus Memory Latency](x86_split_locks_figures/32_figure.png)

![图 33：Goldmont Plus Memory Bandwidth；L1D/L2 小差异也混入未降频导致的 Clock 变化](x86_split_locks_figures/33_figure.png)

![图 34：Goldmont Plus 两项 GB6 的下降，Asset Compression 很可能主要受低频影响](x86_split_locks_figures/34_figure.png)
...
![图 35：Intel 文档对 `LOCK` 与历史 Shared Bus 的说明](x86_split_locks_figures/35_figure.jpg)
...
![图 36：AMD Data Fabric/Coherent Station 相关观测](x86_split_locks_figures/36_figure.jpg)
...
![图 37：Linux 默认 Mitigation 下的 Split Lock Latency](x86_split_locks_figures/37_figure.png)

Multi-user/Server 重视一致性，默认策略合理，可与 Fixed Lower Clock、Cache Partition、Memory Bandwidth Throttle 等 QoS 手段配合。

![图 38：Linux 检测/缓解 Split Lock 时的系统输出](x86_split_locks_figures/38_figure.jpg)
...
![图 39：各平台 Split Lock 自身 Latency 汇总](x86_split_locks_figures/39_figure.png)

![图 40：各平台 Noisy-neighbor 性能影响汇总](x86_split_locks_figures/40_figure.png)
...
![图 41：Strix Halo Split Lock Latency](x86_split_locks_figures/41_figure.jpg)

高性能移动版双 CCD Zen 4 的结果接近 Zen 2；数据由 Owen Hilyard 提供。

![图 42：移动 Zen 4 Split Lock Latency](x86_split_locks_figures/42_figure.jpg)
...
![图 43：在 E-Core 4、5 循环 Split Lock 时的 Bus-lock Blocking Counter](x86_split_locks_figures/43_figure.png)

为选择 GB6 两个代表项目，Arrow Lake Die-to-die Traffic 以 1 秒采样、挑选 Spike 粗略记录；它不是准确的平均 L3 Miss Traffic。

![图 44：Arrow Lake 多项 GB6 的粗略跨 Die Traffic，用于选取 Photo Filter 与 Asset Compression](x86_split_locks_figures/44_figure.png)
