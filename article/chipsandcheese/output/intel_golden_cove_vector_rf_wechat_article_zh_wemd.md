...
themeName: "学术论文 (副本)"
title: "intel_golden_cove_vector_rf_wechat_article_zh"
---
...
![图 1：Skylake-X 向量执行布局；裸片图来自 Fritzchens Fritz](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_golden_cove_vector_rf_wechat_article_zh/5bec61af90e91e3c_01_figure.jpg)

图 1 展示了这种物理不对称。服务器额外浮点单元也像核心扩展一样放在主执行区另一侧，与输入寄存器距离较远，可能是早期 AVX-512 低频、高功耗的因素之一，但裸片距离本身不能单独证明时序瓶颈。

![图 2：Intel Hot Chips 32 展示 Ice Lake-SP 的 AVX-512 负载频率改善](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_golden_cove_vector_rf_wechat_article_zh/bc27944ceb328832_02_figure.png)

工艺与架构迭代逐步缓解频率惩罚，但另一项需求同时增长：乱序执行需要更深窗口隐藏缓存延迟，更多在途结果就要更多物理寄存器。512-bit 寄存器的数据量和布线尤其昂贵。

## 不是所有物理向量寄存器都需要 512 bit

![图 3：三代 AVX-512 Intel 核心的向量寄存器文件局部](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_golden_cove_vector_rf_wechat_article_zh/7b4d74b433480f11_03_figure.jpg)
...
![图 5：第二线程运行不触碰 FP/向量寄存器的整数假负载](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_golden_cove_vector_rf_wechat_article_zh/0423eec24790dbcb_05_figure.png)

结果支持高水位（watermark）方案：两线程活动时，单线程最多约使用 221 个 FP rename；512-bit 操作最多约 141 个。

![图 6：两线程同时运行结构容量微基准时的分配范围](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_golden_cove_vector_rf_wechat_article_zh/bccedcf907c0a700_06_figure.png)
...
![图 7：Zen 4 与 Zen 2 的 FP/向量数据流；绿色为寄存器文件，箭头指向执行单元](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_golden_cove_vector_rf_wechat_article_zh/2cd890c965138472_07_figure.jpg)

Zen 4 对 256-bit 和 512-bit 指令提供相同重命名容量。物理布局把执行单元放到寄存器文件一侧，类似 Intel；此前 Zen 把执行单元分布在两侧，以缩短执行路径，却让 load/store 单元到寄存器更远。

![图 8：Golden Cove、Ice Lake 与 Zen 4 的向量重命名容量对比](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_golden_cove_vector_rf_wechat_article_zh/89d64f98dbd865f1_08_figure.png)
