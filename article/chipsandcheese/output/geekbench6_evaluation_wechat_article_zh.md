...
![图 1：Geekbench 6 的消费级测试套件](geekbench6_evaluation_figures/01_figure.jpg)
...
![图 3：Granite Rapids 与 Ice Lake-X 的动态指令数变化](geekbench6_evaluation_figures/03_figure.png)
...
![图 4：Haswell AVX2 Target 的指令组成](geekbench6_evaluation_figures/04_figure.png)

Ivy Bridge 有 AVX 但 256 bit Math 主要限 FP，故大部分回到 128 bit SSE；256 bit 在多数项低于 1%，只有 Background Blur 大量使用。

![图 5：Ivy Bridge Target](geekbench6_evaluation_figures/05_figure.png)

古老 Prescott x86-64 Base 也以 128 bit SSE 为主，分布竟接近 Ivy Bridge。

![图 6：Prescott Target](geekbench6_evaluation_figures/06_figure.png)
...
![图 7：SPEC CPU2017 的 Vector Instruction Mix](geekbench6_evaluation_figures/07_figure.png)
...
![图 10：Geekbench 6 与 SPEC CPU2017 IPC 分布](geekbench6_evaluation_figures/10_figure.png)

Geekbench 更接近 SPEC FP，但仍更集中；`549.fotonik3d` 是单核 DRAM Bandwidth 限制的低 IPC Outlier。

## Branch：Navigation 是真正例外

![图 11：各 Workload Branch MPKI](geekbench6_evaluation_figures/11_figure.png)

Navigation 远高于其他，Mispredict 很可能是低 IPC 首因；先进 Predictor 有帮助却不能解决，新核相对十年前小核 IPC 提升很少。

![图 12：Navigation 在多代 Core 上的 IPC/MPKI](geekbench6_evaluation_figures/12_figure.png)

File Compression、Clang 中等困难，新 Predictor 在 Clang 更明显。

![图 13：Clang、File/Asset Compression 的 MPKI](geekbench6_evaluation_figures/13_figure.png)
...
![图 14：Lion Cove LSD/Op-cache/Decode Coverage](geekbench6_evaluation_figures/14_figure.png)

Zen 5 有约 6K Op Cache，Clang 仍是唯一显著挑战，且多数时间仍不需 Decoder。这正适合 AMD，因为 Zen 5 Single-thread Decoder 比 Lion Cove 弱。

![图 15：Zen 5 Op-cache Coverage](geekbench6_evaluation_figures/15_figure.png)
...
![图 16：Lion Cove Data Hierarchy MPKI](geekbench6_evaluation_figures/16_figure.png)
...
![图 17：Arrow Lake Tile-to-tile 流量](geekbench6_evaluation_figures/17_figure.png)
...
![图 18：Lion Cove MPKI 与外部流量的反差](geekbench6_evaluation_figures/18_figure.png)

AMD Event 计 Demand Refill，而非在 Retire 给 Load 标 Data Source；被 Mispredict Flush 的 Load 也可能产生 Refill，数值通常更高，不能直接与 Intel 对表。

![图 19：9800X3D Demand Cache Refill](geekbench6_evaluation_figures/19_figure.png)
...
![图 20：9800X3D Memory Controller Traffic](geekbench6_evaluation_figures/20_figure.png)
...
![图 21：各核心相对 Geekbench 2500 Baseline](geekbench6_evaluation_figures/21_figure.png)
