...
RISC-V 是一套相对年轻、开放的指令集。它已经在微控制器、学术研究和各类片上控制器中站稳脚跟：例如，Nvidia 用 RISC-V 替换了 GPU 内部的 Falcon 微控制器，Berkeley BOOM 等大学项目也以 RISC-V 为基础。但从这些场景走向对单线程性能、系统软件和产品成熟度要求更高的市场，难度会陡然增加。SiFive 在这个过程中承担的角色，近似于 Arm 在 Arm 生态中的角色：提供可授权的处理器 IP，由芯片厂商完成 SoC、内存系统和整机实现。成熟的 CPU IP 能降低进入高性能芯片设计的门槛，前提是核心本身足够强。

P550 正是 SiFive 向高性能方向迈进的一步。SiFive 给它定下的目标是：“相对可比的 Cortex-A75，在不到一半面积内提供高 30% 的性能。”这次分析从 SiFive Premier P550 Dev Board 入手，用一组微基准逐层探测它的前端、乱序后端和存储系统。

不过，这类 PPA 宣称离不开工艺、电压、频率、Cache 配置和工作负载。那组受控条件并未给出，不能把公司宣称与下面这块开发板上的测试直接画等号。
...
被测平台是 SiFive Premier P550 Dev Board，具体配置如下：
...
- 特殊对照：图 15 的 Cache 延迟使用 Amlogic S922X 中的 Cortex-A73，原因是 Android 对照机无法使用大页；图 17 的四核带宽也使用该 A73 平台，但没有交代这项选择的原因。

页面开头曾把 SoC 写成 `EC7700X`，其余正文、图题和资料均写 `EIC7700X`。结合这些上下文，首处差异更像是一处笔误；下文统一采用 `EIC7700X`。

测试资料没有披露操作系统与内核版本、编译器、优化参数、微基准完整源码、预热和重复次数、误差范围、线程绑核方式、频率策略及预取器状态。除图 17 明确为四线程/四核、图 21～22 明确为核间测试外，其他图的线程条件也没有逐项完整标注。
...
下文顺着 23 张图展开：先呈现开发板上的测试观察和判断，再讨论这些现象在微架构上可能意味着什么。补充的机制分析统一以“体系结构视角”标出，不代表 P550 已经确认采用某种内部实现。只有图 1、15、18 带英文正式图注，其余 20 条中文说明用于帮助读图。
...
P550 并非 SiFive 第一颗乱序核。更早的 U87 同样采用三宽乱序结构；P550 在数年后问世，理应是一套更成熟的产品。

![图 2：SiFive P550 微架构总览](https://gongzhonghao-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/sifive_p550_wechat_article_zh/ce3ec772e22316bc_02_p550_microarchitecture_overview.png)
...
![图 3：Cortex-A75 微架构总览](https://gongzhonghao-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/sifive_p550_wechat_article_zh/181c005b14692104_03_cortex_a75_microarchitecture_overview.png)

*图 3：Cortex-A75 对照图同样混合了公开资料与反推值。前端为约 48 项 L0 BTB、3072 项 L1 BTB、约 42 项 RAS、64 KB/4 路 L1I 和三宽译码；后端约有 73 项 ROB、约 101 项整数与 89 项 FP/NEON 寄存器、69 项 Load Buffer、14 项 Store Buffer，并另画出 Branch/Store Completion Queue。整数、分支、AGU 与 FP 调度分区标为 12、12、20、8、8、16、16 项，两条 FP/NEON 路径中一路还标出 AES。数据侧为 64 KB/4 路 L1D、双 8 B/cycle 读和单 8 B/cycle 写。后面的文字却称 A75“似乎只有一个小型 BTB 层级”，因此精确 BTB 层级仍无法确定。*

P550 与 A75 的乱序引擎虽然都远小于当代 Intel、AMD 高性能大核，却不能简单归结为“落后”。两者只是把面积和功耗预算放在目标市场更需要的位置；真正需要考察的，是相邻结构是否匹配，以及不常见的慢路径是否足够成熟。

### 体系结构视角：三宽真正考验的是整条供给链
...
分支预测的速度和准确率会同时影响性能与能效。SiFive 资料给出的分支历史表（Branch History Table，BHT）容量为 9.1 KiB，用历史行为帮助判断条件分支是 Taken 还是 Not Taken。

测试构造了不同静态分支数量、不同随机模式长度的条件分支序列。图中的高度轴是可预测模式与随机模式之间的时间差，曲面的明显转折被用作模式识别边界的经验信号。P550 在静态分支较少时能识别相当长的模式；随着分支数量增加，它与 A75 的差距逐渐缩小。这样的模式识别能力符合 P550 的产品定位，但与当代高性能大核仍有明显距离。
...
较合理的一种解释是：P550 只有约 32 项快速分支目标缓冲区（Branch Target Buffer，BTB），没有更大的解耦二级 BTB；miss 后，前端需要等待分支被识别，再计算 PC 相对目标。如果这一解释成立，约 3 周期的台阶还暗示 L1I 延迟可能也是 3 周期。不过，这仍是一套基于微基准的反推，并非 RTL 结论。
...
*图 7：在 16 B 分支间距下，P550 和 A75 都先经历约 32 项附近的延迟台阶；更大工作集中的曲线又受取指层级影响。*

从这组对照看，两颗核心似乎都缺少现代高性能大核常见的大容量、解耦式目标供给结构。不过，图 3 又把 A75 画成约 48 项 L0 BTB 加 3072 项 L1 BTB；两处材料并不一致，因此 A75 的精确 BTB 层级仍无法确定。

函数返回是第三种目标问题。同一个 `ret` 指令会因调用者不同而回到不同地址，因此处理器通常用返回地址栈（Return Address Stack，RAS）追踪动态调用嵌套。

P550 的曲线在调用深度超过 16 后陡然上升，支持其 RAS 约为 16 项的判断。A75 要到调用深度超过约 42 才出现拐点，据此估计其 RAS 约有 42 项；更高的 2 GHz 频率也让 A75 在常见命中路径上的绝对时间与 P550 接近。容量溢出后，A75 的增长更平缓。一种可能是，A75 只错掉被挤出的返回项，而 P550 的溢出处理不够平滑，少量超深嵌套也可能引发连续 return 误预测。这只是对曲线的机制假说，没有硬件证据。
...
公开规格给出的 L1 指令 Cache 为 32 KB、4 路组相联，带奇偶校验保护，可以向下游三宽译码器提供 12 B/cycle。代码位于 L1I 时，指令取数微基准接近这一上限，并观察到约 3 IPC 的持续供给能力。

工作集超过 L1I 后，取指字节带宽降到约 8 B/cycle，折合约 2 IPC。工作集进入 L3 后，约 4 B/cycle 对应约 1 IPC；再向下进入内存，供给接近 1 B/cycle。任何核心都希望 L2 代码 miss 足够少，因此这些台阶更适合用来观察层次，而不是当作常态吞吐。
...
可见容量是通过阻塞退休来探测的。A75 的测试明确采用一条未完成分支与一条未完成 Load 的组合；P550 的具体阻塞序列则没有交代。微基准给出的 P550 ROB 约为 96 项，高于 A75 的约 73 项；但 A75 继承了 A73 的特殊乱序退休行为，某些阻塞构造下可以让缓冲资源利用得更充分。因此，这里测到的是特定条件下的可见容量，不是所有工作负载中的等效窗口。
...
*图 11：P550 三条整数路径的调度容量标为 16、约 8、16 项，独立 Load/Store AGU 各约 18 项，FP 合计约 28 项；A75 的两条 ALU、独立分支端口、两条通用 AGU 和两条 FP/NEON 路径分别对应 12、12、20、8、8、16、16 项。方框中的容量来自微基准；P550 第二整数调度器的 8 项带问号，FP 的 28 项也无法区分统一双端口队列与两个 14 项队列。*

P550 的整数端口可达性更灵活，调度容量也更多；A75 的独立分支端口不需要整数寄存器写回路径，可能以更低成本获得接近的标量整数性能。这是对设计取舍的判断，并非面积测量。
...
A75 的 FP 加法与乘法为 3 周期，FMA 为 5 周期，可能使用分离单元，也可能只是在共享单元中设计了更短旁路。A75 还支持 128 位 NEON 向量执行，而 P550 没有向量能力。微基准测得 A75 约有 31 项 FP 调度容量，与 AnandTech 给出的两个 8 项调度器并不一致；P550 则约为 28 项。测试没有找到只能送往其中一条 P550 FP 路径的操作，因此仍无法区分统一双端口调度器与两个 14 项调度器。
...
端口冲突测试呈现出两条可区分的地址生成路径（Address Generation Unit，AGU）：一条只处理 Load，另一条只处理 Store；两者后面都约有 18 项调度容量。A75 的两条 AGU 都能处理 Load 或 Store。由于程序中的 Load 通常远多于 Store，P550 可能出现 Load AGU 很忙、Store AGU 却大多空闲的情况；A75 的组织对此更灵活。

David B. Papworth 在 *Tuning the Pentium Pro Microarchitecture* 中记录过一个相似取舍：Intel 观察到两条 Load/Store 端口的平均利用率都只有约 20%，改成一条专用 Load 和一条专用 Store 后，性能损失不到 1%。SiFive 可能也得出了类似结论。P550 的重排序容量又大于 Pentium Pro，理论上更有机会在 Cache miss 期间继续为包括 AGU 在内的执行管线寻找工作；但这只是设计类比，不代表 SiFive 采用了同一套分析过程。
...
上千周期的代价和额外指令数，强烈指向“触发异常、再由操作系统处理程序模拟”，也意味着 P550 很可能没有硬件未对齐访问支持。不过，内核实现、固件路径和计数器口径都可能影响结果，目前仍没有 RTL 或处理程序路径佐证，因此这里仍按强推断处理。
...
即使按低功耗核心的标准，P550 的未对齐访问表现也很反常。作为对照，Cortex-A75 在 Load 与 Store 相互依赖且两者都未对齐的最坏情况下，也只需约 15 周期。

### 体系结构视角：慢路径同样定义一颗处理器的成熟度
...
P550 没有向量能力，乱序窗口也不大，其目标负载和单核结构通常不会要求与宽向量大核相同的峰值存储吞吐；但它更难用大量独立工作隐藏 miss，Cache 命中延迟反而格外重要。和许多 AMD、Intel、Arm 设计一样，每个 P550 核拥有私有 L1/L2，L3 则由核心共享；公开资料还显示，数据侧各级 Cache 都有 ECC 保护。

P550 的 32 KB L1D 为 4 路组相联，命中延迟约 3 周期。在没有对齐问题时，它每周期可以服务一条 64 位 Load 和一条 64 位 Store，读写各 8 B/cycle，均衡读写的合计峰值为 16 B/cycle。

私有 L2 为 256 KB、8 路组相联、双 bank，命中延迟约 13 周期。与 Arm、AMD 已经实现的更大容量、更低延迟 L2 相比，这个容量—延迟组合已经有些陈旧；不过，它仍能在相当一部分 L1 miss 到达共享 L3 之前将其截住。读写带宽都约为 8 B/cycle，只能算平平，但考虑到 P550 没有向量能力且只有一条 Load AGU，仍可能满足目标负载。
...
*图 15：英文图注说明，由于 Android 平台无法使用大页，这里改用 A73 对比。测试使用 2 MB 页；P550 的台阶约为 L1D 3.01 cycle、L2 13.06 cycle、L3 38.11 cycle；Amlogic S922X 中 Cortex-A73 的 L1 约 3 cycle、1 MB L2 约 24.94 cycle。*
...
从这组测试看，A75 在每周期带宽和频率上都更高，因此各级 Cache 的绝对带宽领先 P550。P550 的 8 B/cycle L2 并不亮眼，但与其单 Load AGU、无向量和低频定位基本匹配。
...
P550 的互连必须兼顾模块化和扩展性：消费级路由器或机顶盒可能只需要 2～4 核，小型边缘服务器则可能需要更多核心。P550 最多以四核组成一个 cluster，多个 cluster 可以接入 Coherent System Fabric。结合 EIC7700X 数据表和微基准曲线，一种可能的组织方式是用 crossbar 连接各端口，并让单个 cluster 共享一个外部接口；这仍是结构反推。
...
*图 17：该图跨越私有 L1/L2、共享 L3 与 DRAM。近端区间里，四核 P550 的 read 约为 43～44 GB/s、Add 约为 75～77 GB/s，四核 A73 的 read 约为 119～121 GB/s、Add 约为 108～110 GB/s。四线程 P550 的 Add 曲线在共享 L3 区间明确标出约 43.88 GB/s；图中另一个 43.17 标注位于多条曲线交叠的过渡区域，无法稳妥确定归属。远端 P550 read 标出 16.74 GB/s，A73 read 标出约 8.02 GB/s。测试没有定义 Add 带宽是否同时计入读写流量，因此不同访问模式不能直接换算成端口宽度。*

![图 18：微基准反推的 P550 存储层次与 Coherent Fabric](https://gongzhonghao-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/sifive_p550_wechat_article_zh/f7940039240dab96_18_p550_memory_hierarchy.png)
...
微基准测得单核 L3 带宽约为 8 B/cycle，四核合计约 43.88 GB/s；L3 延迟约为 38 周期，对一套可配置、可共享的缓存来说并不差。作为对照，A73 的 1 MB L2 同时承担 L1D miss 的下一站与末级 Cache：容量小于 EIC7700X 的 L3，但约 25 周期的延迟更低。
...
开发板的 DRAM load-to-use 延迟约为 194 ns，比 L3 多出约 165 ns。现有测试无法区分其中多少来自片上网络，多少来自内存控制器。与 Meteor Lake 和 AMD Van Gogh（Steam Deck SoC）等 LPDDR5 平台作定性对照，EIC7700X 的延迟明显偏高；由于方法和配置并不统一，这不是严格的横向排名。

DRAM 实测带宽为 16.74 GB/s，明显低于 64 位 LPDDR5-6400 的理论值。inline ECC 会占用一部分位宽，却不足以解释全部差距。不过，对四个 1.4 GHz、没有向量单元的低功耗核心，这一带宽在许多场景中仍可能够用。
...
核间延迟采用一套为这次分析专门编写的 core-to-core latency 测试，用来记录一个核心多久能观察到另一个核心的写入。具体方法没有公开，而且与 AnandTech 等站点的测试并不完全相同，因此两边的结果只能作大致对照。
...
Snapdragon 670 的结果明显更好，即使传输发生在 A75 与 A55 之间也是如此。EIC7700X 的高核间延迟未必会明显影响一般应用，却让 P550 平台显得“不够精致”。
...
RISC-V 仍然年轻，SiFive 进入 CPU 设计行业的时间也不长；高性能通用 CPU 的参与者寥寥，本身就说明这项工作有多难。P550 的目标是在严格的功耗和面积约束下取得尽可能高的性能，而不是正面对抗 Zen 5、Lion Cove 或 Oryon。它的乱序引擎规模更接近早期 Core 2，频率又显著更低，因此更像一颗低功耗、性能适中的核心，适合承担顺序核略显吃力的轻量管理任务。这是由一系列微基准形成的产品判断，不能据此给适用场景下定论。
...
相比横向排名，P550 的演进意义更加重要。SiFive 不久前还主要聚焦微控制器所需的小型顺序核，而 P550 已经建立起一套相对均衡的乱序引擎和称职的 Cache 层次。把这些基础链路做完整，其重要性怎么强调都不为过。高性能通用处理器很难绕开乱序执行：Intel 与 IBM 都曾因复杂度尝试在 Itanium、POWER6 上走向不同路线，最终仍说明从动态执行中提取指令级并行极其重要。就此而言，SiFive 的进展令人乐观。

不过，P550 仍只是中间一步。与 Cortex-A75 相比，它更像 Arm 较早期的 Cortex-A57：时钟较低、未对齐访问代价异常高、缺少向量支持，很多边角路径尚未打磨。A75 推出时，Arm 已积累多年乱序核心经验，因此整体更完整。另一方面，最坏慢路并非所有程序都会触发，不能用这些极端微基准否定全部常见工作负载；但若要真正进入高性能市场，SiFive 仍有很长的路要走。

P550 还可以类比为 Intel 的 Pentium Pro。Pentium Pro 在 16 位代码等场景中表现不佳，某些重要角落也不够精致，却仍是一套整体设计良好的核心，并让 Intel 建立了继续挑战更复杂 CPU 的信心。SiFive 后续已经公布 P870 等更复杂方案；P550 为这些核心积累了基础，后续实现值得期待。

## 十、体系结构视角：从 P550 可以看到的六件事

结合这组测试和前面的机制分析，还可以归纳出六点设计认识。这里关注的是各机制之间的关系；其中带“约”的容量和端口组织，仍以微基准反推为限。
...
**4. 分支前端不只怕方向猜错，也怕目标来不及。** P550 的方向模式识别大体符合其定位；“约 32 项快速目标容量”则是用来解释更大跳转分支工作集出现约三周期台阶的一种结构推断。方向、BTB、RAS、取指块和恢复链必须共同设计；只报一个分支准确率，会漏掉目标气泡和级联恢复代价。
...
如果希望支持 Chips and Cheese，可以通过英文页面中的 Patreon 或 PayPal 入口捐助，也可以加入其 Discord 社区。
