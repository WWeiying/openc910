...
![图 1：Golden Cove 48 KB/5 周期 L1D 与 32 KB/4 周期假设配置](intel_golden_cove_cache_figures/01_figure.png)

把 L1D 改成类似 Skylake 的 32 KB、4 周期是现实可想象的配置：Skylake 在较旧工艺仍可超过 5 GHz，Zen 3 的 32 KB/4 周期也能高频运行。

![图 2：32 KB/4 周期相对基线的逐 trace IPC 变化](intel_golden_cove_cache_figures/02_figure.png)

214 条中有 204 条由更小、更快的 L1D 获胜。

![图 3：两种 L1D 的命中率对比](intel_golden_cove_cache_figures/03_figure.png)
...
![图 4：Phenom 风格 L1D 与 Golden Cove 基线参数](intel_golden_cove_cache_figures/04_figure.png)

Phenom 曾用更大、更快但低相联、低带宽的 L1D。假设现代工艺能让 64 KB、3 周期、2-way 配置跑到 Golden Cove 频率，可以检验更激进的延迟路线。

![图 5：Phenom 风格 L1D 的 IPC 结果](intel_golden_cove_cache_figures/05_figure.png)

它在 207/214 条 trace 获胜，通常比 32 KB/4 周期进一步提升。

![图 6：少数 trace 的严重负收益](intel_golden_cove_cache_figures/06_figure.png)

最大两个失败来自用途未知的 Qualcomm trace；SPEC2006 `410.bwaves` 两条分别下降 9.18% 和 6.86%，命中率仅下降不到 2%，MPKI 却从 2.03 增至 3.03。

![图 7：64 KB 与 48 KB L1D 命中率](intel_golden_cove_cache_figures/07_figure.png)

64 KB 并没有稳定胜过 48 KB，罪魁祸首是 2-way 相联：热点地址更容易映射到同一 set，冲突 miss 抵消容量优势。

![图 8：L1D 配置的容量、相联度、延迟和表现汇总](intel_golden_cove_cache_figures/08_figure.png)
...
![图 9：Golden Cove 1.25 MB/15 周期与 Zen 风格 512 KB/12 周期](intel_golden_cove_cache_figures/09_figure.png)

![图 10：小而快 L2 的逐 trace IPC](intel_golden_cove_cache_figures/10_figure.png)

结果是否定的：115/214 条 trace 变慢。容量扩大约 2.5 倍带来的命中率收益足以覆盖 3 周期惩罚。

![图 11：两种 L2 的命中率](intel_golden_cove_cache_figures/11_figure.png)

![图 12：两种 L2 的 miss per instruction](intel_golden_cove_cache_figures/12_figure.png)

程序访问 L1D 的频率远高于 L2，L2 延迟只作用于 L1 miss；而 Golden Cove L2 miss 又要进入高延迟 L3，比 Zen 上的代价更大，因此避免 miss 更重要。

![图 13：小 L2 结果分布，少数未知 Qualcomm trace 形成极端负值](intel_golden_cove_cache_figures/13_figure.jpg)

其中几条 trace 的 L2 命中率从 90% 以上跌至约 50%，显著拉低平均数，且应用身份未知。

![图 14：去掉极端值后的结果分布](intel_golden_cove_cache_figures/14_figure.jpg)

其余 trace 中确有不少受益于 12 周期，但收益更小、数量也更少。Golden Cove 的大 L2 能把核心隔离在高延迟 L3 之外，是缓存层级中的亮点。

### Willow Cove 风格：20-way、14 周期

![图 15：Willow Cove 与 Golden Cove L2 参数](intel_golden_cove_cache_figures/15_figure.png)

![图 16：20-way、14 周期 L2 的 IPC 增益](intel_golden_cove_cache_figures/16_figure.png)

它几乎全面小幅领先，但幅度很小。

![图 17：提高相联度后的 L2 命中率](intel_golden_cove_cache_figures/17_figure.png)

![图 18：提高相联度后的 L2 MPKI](intel_golden_cove_cache_figures/18_figure.png)
...
![图 19：Golden Cove 基线与 Zen 3 风格 L3](intel_golden_cove_cache_figures/19_figure.png)

Zen 自第一代起使用与核心频率紧密耦合的低延迟 L3；Intel 从 Haswell 起让 ring/L3 使用独立 uncore 频率，以免 iGPU 需要缓存带宽时迫使核心也高频运行。

![图 20：更大、更快、更高相联 L3 的 IPC 变化](intel_golden_cove_cache_figures/20_figure.png)
...
![图 21：Zen 3 风格 L3 的结果分布](intel_golden_cove_cache_figures/21_figure.jpg)
...
![图 22：Intel 早期 128 MB eDRAM 与 AMD 3D V-Cache 的背景](intel_golden_cove_cache_figures/22_figure.jpg)

![图 23：假设 90 MB、略高延迟 L3 的平均 IPC](intel_golden_cove_cache_figures/23_figure.png)
...
![图 24：90 MB L3 的 hitrate/IPC 关系](intel_golden_cove_cache_figures/24_figure.png)

![图 25：90 MB L3 的逐 trace 分布](intel_golden_cove_cache_figures/25_figure.jpg)
...
![图 26：两套完整缓存层级参数](intel_golden_cove_cache_figures/26_figure.jpg)

![图 27：Zen 3 全套缓存带来的 IPC 变化](intel_golden_cove_cache_figures/27_figure.png)

多数情况下，低延迟 L1D/L3 的收益超过 L2 从 1.25 MB 缩到 512 KB 的损失，但结果不统一。

![图 28：IPC 变化与 L2 miss 增量呈负相关](intel_golden_cove_cache_figures/28_figure.png)
...
![图 29：Golden Cove 与 Apple M1 Max 缓存参数](intel_golden_cove_cache_figures/29_figure.jpg)

这完全不现实：M1 的频率目标和使用场景不同。但它能显示缓存本身的性能上限。

![图 30：M1 Max 缓存配置的逐 trace IPC](intel_golden_cove_cache_figures/30_figure.png)

205/214 条 trace 提升，约一半提升超过 10%，仅缓存层级就接近一次代际 IPC 增长。

![图 31：M1 大 L1 带来的命中率优势](intel_golden_cove_cache_figures/31_figure.png)

M1 的 192 KB L1I + 128 KB L1D 合计 320 KB，甚至超过 Nehalem 到 Skylake 的 256 KB L2；L1D 仍为 3 周期，像一个命中率更好的 Phenom L1D。

![图 32：M1 Max 与 Golden Cove L2 的 miss per instruction](intel_golden_cove_cache_figures/32_figure.png)
...
![图 33：Alder Lake 与 Sandy Bridge-E/客户端的 L3 延迟](intel_golden_cove_cache_figures/33_figure.png)
...
![图 34：ChampSim 基线的核心、缓存与内存参数](intel_golden_cove_cache_figures/34_figure.png)
...
总体上，Golden Cove 的大 L2 是正确取舍；高延迟 L3 最值得改进；巨大堆叠缓存只对特定大工作集产生跳跃式收益；M1 缓存展示了低频、窄目标设计的另一种上限。缓存优化没有单一答案，物理频率、端口、能耗和目标软件必须一起决定。
