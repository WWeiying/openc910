...
![图 1：CPU-Z 的处理器信息与内置 Benchmark 界面；识别结果本身也可能出现古怪之处](cpu_z_benchmark_figures/01_figure.png)
...
![图 2：CPU-Z 动态指令组成，以标量 SSE 浮点操作为主](cpu_z_benchmark_figures/02_figure.png)

SSE 编码和大位移内存操作数使平均指令长度达到 4.85 Byte。

![图 3：指令长度分布；较长的 x86 指令占据显著比例](cpu_z_benchmark_figures/03_figure.png)

对 L1I 每周期只能供给 16 Byte 的老 Intel 核而言，理论上可能先碰到取指带宽；但只要代码被 Micro-op Cache（微操作缓存）覆盖，这个限制就不会出现，而且执行端也得先达到足够高的 IPC。

![图 4：CPU-Z 的 Load、Store 与 Branch 比例；分支密度低于游戏、压缩和 Cinebench 2024](cpu_z_benchmark_figures/04_figure.png)

## 总体画像：前端轻松，后端等待计算

除 Bulldozer 外，各核心大致只能达到自身宽度的一半以下，属于中等 IPC 负载。

![图 5：多种微架构运行 CPU-Z 单线程测试时的 IPC](cpu_z_benchmark_figures/05_figure.png)
...
![图 6：Goldmont Plus、Kaby Lake 与 Zen 4 的流水槽位去向](cpu_z_benchmark_figures/06_figure.png)
...
![图 7：后端受限周期的构成，核心多在等待计算完成](cpu_z_benchmark_figures/07_figure.png)

![图 8：数据缓存命中率；主要访问停留在 L1D](cpu_z_benchmark_figures/08_figure.png)
...
![图 9：Bulldozer 后端结构占满情况；每线程 128 项 ROB 频繁成为边界](cpu_z_benchmark_figures/09_figure.png)

![图 10：Bulldozer FP Port 利用率不高，表现更像延迟受限而非吞吐端口受限](cpu_z_benchmark_figures/10_figure.png)
...
![图 11：Kaby Lake 后端结构压力；多数项目来自未公开计数器](cpu_z_benchmark_figures/11_figure.png)

它的 224 项 ROB 能保留更多独立工作，基本 FP 延迟通常为 4 Cycle，也比 Bulldozer 的 5 Cycle 更快穿过依赖链。

![图 12：Kaby Lake 执行端口利用率；两条 FP Pipe 较忙但没有饱和](cpu_z_benchmark_figures/12_figure.png)

Zen 2 的 FP 加乘延迟进一步降到 3 Cycle。36 项 FP Scheduler 前还有 64 项非调度队列，可追踪约 100 个未完成 FP Operation，因而压力转移到同为 224 项的 ROB。

![图 13：Zen 2 的 FP 队列与 ROB 占满情况](cpu_z_benchmark_figures/13_figure.png)

![图 14：Zen 2 FP Pipe 利用率；FP2 还承载 FP Store，负载略高](cpu_z_benchmark_figures/14_figure.png)
...
![图 15：Zen 4 FP 调度容量压力](cpu_z_benchmark_figures/15_figure.png)

和 Zen 2、Bulldozer 的 ROB 先满不同，Zen 4 更早耗尽 192 项 FP Register File；Zen 2/3 为 160 项。这说明绕过依赖链找到的独立指令本身也多是 FP，需要长期占用物理寄存器。

![图 16：Zen 4 后端受限来源，FP Register File 成为突出瓶颈](cpu_z_benchmark_figures/16_figure.png)
...
![图 17：Kaby Lake、Zen 2、Zen 4 的 Micro-op 来源；纵轴从 75%起](cpu_z_benchmark_figures/17_figure.png)
...
![图 18：Sandy Bridge 发布时 Intel 给出的典型 Micro-op Cache 命中率约为 80%](cpu_z_benchmark_figures/18_figure.png)
...
![图 19：各核心 L1I 命中情况；代码容量几乎不构成挑战](cpu_z_benchmark_figures/19_figure.png)

## 分支：少、容易预测，BTB 也装得下

CPU-Z 的分支更少、更规律。Bulldozer 相对困难，但误预测仍是次要问题；现代 Intel、AMD 准确率接近，AMD 投入更大面积的 Predictor 在此得不到相称回报。

![图 20：单/多线程分支误预测率；注意各子图量程不同](cpu_z_benchmark_figures/20_figure.png)
...
![图 21：CPU-Z 的 BTB 层级命中/重定向情况](cpu_z_benchmark_figures/21_figure.png)
...
![图 22：AMD 幻灯片中的 Zen 4 对 Zen 3 IPC 增幅，CPU-Z 仅约 1%](cpu_z_benchmark_figures/22_figure.png)

![图 23：CPU-Z 跑分界面与参考结果](cpu_z_benchmark_figures/23_figure.png)
