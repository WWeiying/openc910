...
![图 1：Skylake 及其大量 Refresh](intel_skylake_figures/01_figure.png)
...
![图 2：Client Skylake 核心](intel_skylake_figures/02_figure.png)

*图 2：Skylake 扩大 Buffer、调整 Scheduler/RF，并增强向量与 Fetch，但仍是四宽 Rename 的 Core。*

![图 3：Haswell 核心对照](intel_skylake_figures/03_figure.png)
...
![图 4：分支 Pattern 测试之一](intel_skylake_figures/04_figure.png)

*图 4：Skylake 在长 Pattern 下没有显著改变可见行为。具体内部优化仍可能存在，微基准不一定敏感。*

![图 5：分支 Pattern 测试之二](intel_skylake_figures/05_figure.jpg)

*图 5：英文正式图注指出 Skylake Direction Predictor 很像 Haswell。*

![图 6：实际负载 MPKI](intel_skylake_figures/06_figure.png)
...
![图 7：BTB Footprint 与 Taken Branch 代价](intel_skylake_figures/07_figure.png)
...
![图 8：Skylake Die 上的 Predictor 区域](intel_skylake_figures/08_figure.jpg)
...
![图 9：Skylake Frontend Die 区域](intel_skylake_figures/09_figure.jpg)
...
![图 10：大 Instruction Footprint 吞吐](intel_skylake_figures/10_figure.png)

*图 10：从 L2/L3 Fetch 时 Skylake 明显占优，可能来自更高 Instruction-side MLP 与更积极 Prefetch。*

![图 11：4-byte NOP 的 Instruction Bandwidth](intel_skylake_figures/11_figure.png)
...
![图 12：Move/Zero Elimination](intel_skylake_figures/12_figure.jpg)

*图 12：Rename Optimization 可以省执行资源，但长依赖链等模式下 Move Elimination 并非总能成功。*

![图 13：Skylake 与 Haswell 乱序资源容量](intel_skylake_figures/13_figure.jpg)
...
![图 14：Skylake Backend Die 区域](intel_skylake_figures/14_figure.jpg)
...
![图 15：Skylake 的 RF 变化](intel_skylake_figures/15_figure.png)

*图 15：单独 RF 也释放主 256-bit Vector RF 的容量；Client 没有 AVX-512，因此承担工程与面积成本却用不到主要目的。*

![图 16：Haswell Backend 对照](intel_skylake_figures/16_figure.jpg)

*图 16：正式图注要求注意新增 x87/MMX RF 与分离 Scheduler；Haswell Die Photo 来自 Cole L。*

![图 17：Backend Resource Stall](intel_skylake_figures/17_figure.png)
...
![图 24：Cache Latency](intel_skylake_figures/24_figure.png)

*图 24：Skylake/Haswell Absolute L3 低于 10 ns、L2 略高于 3 ns；超过 L2 时曲线更平缓，暗示 Replacement Policy 改变。Piledriver 各级更慢。*

![图 25：4 KB Page 下的 Translation+Cache Latency](intel_skylake_figures/25_figure.png)
...
![图 30：Skylake-X Mesh Latency](intel_skylake_figures/30_figure.png)

*图 30：Mesh L3 Latency 明显升高，Topology 扩展性不是免费收益。*

![图 31：Skylake-X L3 Bandwidth](intel_skylake_figures/31_figure.png)
...
![图 32：Skylake-X 与 Client 差异](intel_skylake_figures/32_figure.jpg)

*图 32：正式图注为 Skylake-X 标注图，照片来自 Fritzchens Fritz。*

![图 33：为 Server Variant 预留的核心变化](intel_skylake_figures/33_figure.jpg)

*图 33：AVX-512 让 L1D 每周期支持 2×512-bit Load+1×512-bit Store，带宽是 Client Skylake 两倍，虽然 Server Clock 通常更低。*

![图 34：Client IPC 世代提升](intel_skylake_figures/34_figure.jpg)
...
![图 35：Kaby Lake Power/Frequency](intel_skylake_figures/35_figure.png)

*图 35：正式图注说明 Kaby Lake 可用更高功耗换 Clock，也在低功耗区扩展更好。*

![图 36：六核 Coffee Lake Die](intel_skylake_figures/36_figure.jpg)

*图 36：Intel 利用 Modular Ring/L3 Slice 增至六核；照片来自 Fritzchens Fritz。*

![图 37：八核 Coffee Lake Refresh Die](intel_skylake_figures/37_figure.jpg)

*图 37：继续增至八核，更大 L3 也帮助低线程。*

![图 38：四到十 Ring Stop 的 L3 延迟](intel_skylake_figures/38_figure.png)

*图 38：正式图注上方 ns、下方 cycle，Comet Lake 数据由 Mohamexiety 测试。四到十 Stop 增约 11 cycle，Absolute 从 8～9 ns 到略高于 10 ns，仍很合理。*

![图 39：L3 扩展的另一种表示](intel_skylake_figures/39_figure.png)
...
![图 40：Zen 1/2 与 Skylake Cache Latency](intel_skylake_figures/40_figure.png)

*图 40：Zen 2 L3 翻倍，TSMC 7 nm 帮助增频，3950X L3 略高于 8 ns，并让单线程可见 16 MB。*

![图 41：Cache Bandwidth（Absolute）](intel_skylake_figures/41_figure.png)

*图 41：正式图注说明上方 Absolute、下方按 Core Cycle 归一化，以校正 EPYC 较低 Clock。*

![图 42：Cache Bandwidth（归一化）](intel_skylake_figures/42_figure.png)

*图 42：Zen 2 全宽 256-bit Vector 追平 Intel 的 L1D 优势，L3 在多方面超过 Skylake。*

![图 43：Backend 容量的代际追赶](intel_skylake_figures/43_figure.jpg)

*图 43：Zen 逐代扩大 Buffer；Renamer 可在长依赖链消除 MOV，5 Instruction/6 Micro-op 每周期，使其有效五宽，而 Skylake 仍四宽。*

![图 44：Op Cache 与大 Footprint Fetch](intel_skylake_figures/44_figure.png)
...
![图 45：Taken Branch Handling 对照](intel_skylake_figures/45_figure.png)
...
![图 46：Skylake 后端 Die](intel_skylake_figures/46_figure.jpg)
