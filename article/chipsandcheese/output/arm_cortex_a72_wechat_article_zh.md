...
![图 1：Cortex-A72 核心总览](arm_cortex_a72_figures/01_cortex_a72_overview.png)
...
![图 2：Cortex-A72 的重复模式识别](arm_cortex_a72_figures/02_a72_branch_pattern.png)

*图 2：同时改变 Pattern Length、Taken 数和静态分支足迹。A72 对较长重复模式的识别在同代低功耗核中尚可，低平台后的台阶混入历史长度、容量和混叠。*

![图 3：Snapdragon 821 Kryo 的重复模式识别](arm_cortex_a72_figures/03_kryo_branch_pattern.jpg)

*图 3：同类测试下，Kryo 在较长 Pattern 上更早出现不稳定区域；A72 似乎有略多的历史存储，静态分支很多时也稍晚受混叠影响。*

![图 4：Neoverse N1 的重复模式识别](arm_cortex_a72_figures/04_n1_branch_pattern.png)
...
![图 5：A72 的间接分支容量](arm_cortex_a72_figures/05_a72_indirect_targets.jpg)

*图 5：横轴改变目标数，另一维改变分支数。约 256 个总目标以内存在较稳定区域，但“128×2”和“单分支 16 目标”是不同测试切片，不能相乘成物理表容量。*

它还有一个反常现象：分支不超过四个、每分支目标也不超过四个时，反而容易误预测。

![图 6：少量分支和目标下的异常区域](arm_cortex_a72_figures/06_a72_indirect_short_pattern.jpg)
...
![图 7：加入附近直接分支相关性后的间接预测](arm_cortex_a72_figures/07_a72_indirect_correlated_history.jpg)
...
![图 8：正确 Call/Return 对的基本代价](arm_cortex_a72_figures/08_call_return_latency.jpg)

*图 8：正确预测的一对 Call + Return 约 4 周期，平均每条分支约 2 周期。*

![图 9：A72 的返回栈深度](arm_cortex_a72_figures/09_return_stack_depth.png)
...
![图 10：Graviton 1 的 A72 BTB 容量](arm_cortex_a72_figures/10_btb_capacity.jpg)
...
![图 11：Snapdragon 821 Kryo 的 BTB 层级](arm_cortex_a72_figures/11_kryo_btb_latency.png)
...
![图 12：Cortex-A72 前端](arm_cortex_a72_figures/12_cortex_a72_frontend.png)
...
![图 13：A72 与 Kryo 的取指带宽](arm_cortex_a72_figures/13_instruction_fetch_bandwidth_bytes.jpg)
...
![图 14：同一取指测试换算为 IPC](arm_cortex_a72_figures/14_instruction_fetch_bandwidth_ipc.jpg)

*图 14：AArch64 每条指令固定 4 B，L1I 内 A72/Kryo 约 2.65/3.94 IPC；进入 L2 后约 0.61/0.91 IPC，内存约 0.27/0.06 IPC。两者从 L2 都无法维持 1 IPC。*

![图 15：Kryo 前端作为对照](arm_cortex_a72_figures/15_kryo_frontend.png)
...
![图 16：A72 的乱序资源分配](arm_cortex_a72_figures/16_a72_reordering_resources.jpg)
...
![图 17：不同资源的实测重排序容量](arm_cortex_a72_figures/17_measured_reordering_capacity.jpg)
...
![图 18：A72 与 Jaguar、Silvermont 的低功耗乱序容量](arm_cortex_a72_figures/18_low_power_ooo_comparison.jpg)
...
![图 19：Haswell 单线程整数 Rename 容量的异常尖峰](arm_cortex_a72_figures/19_haswell_integer_rename_capacity.png)
...
![图 20：A72 与 Kryo 的整数 Scheduler](arm_cortex_a72_figures/20_integer_execution.png)
...
![图 21：A72 与 Kryo 的 FP/Vector 端口](arm_cortex_a72_figures/21_fp_vector_layout.png)
...
![图 22：FP/Vector 吞吐与延迟](arm_cortex_a72_figures/22_fp_vector_execution.jpg)
...
![图 23：A72 与 Kryo 的 AGU 布局](arm_cortex_a72_figures/23_agu_layout.png)

*图 23：A72 的固定 Load/Store 绑定简化 Rename 分配；Kryo 需要决定 Load 绑定哪条端口，却换来更高 Load 吞吐与更灵活调度。*

![图 24：更大 AGU Scheduler 在 Zen 2 工作负载中的意义](arm_cortex_a72_figures/24_agu_scheduler_comparison.png)
...
![图 25：A72 的 Store-to-Load Forwarding 矩阵](arm_cortex_a72_figures/25_a72_store_forwarding.jpg)
...
![图 26：Kryo 的 Store Forwarding](arm_cortex_a72_figures/26_kryo_store_forwarding.jpg)

*图 26：Kryo 能识别所有 Load 被 Store 包含的组合，却要约 15～16 周期，部分重叠再多 1 周期，远高于 A72。*

![图 27：放大低延迟差异后的 Kryo 矩阵](arm_cortex_a72_figures/27_kryo_store_forwarding_detail.jpg)
...
![图 28：A72 与 Kryo 的 Load/Store 路径汇总](arm_cortex_a72_figures/28_load_store_paths.png)
...
![图 29：Graviton 1 的 A72 Cache/内存延迟，ns](arm_cortex_a72_figures/29_a72_cache_memory_latency_ns.jpg)
...
![图 30：同一延迟按 2.3 GHz 换算为周期](arm_cortex_a72_figures/30_a72_cache_memory_latency_cycles.jpg)
...
![图 31：A72 与 Kryo 的 4 KB 页实际延迟](arm_cortex_a72_figures/31_a72_kryo_latency_ns.jpg)

*图 31：A72/Kryo L1 约 5.53/1.29 ns，L2 约 9.16/10.94 ns，最大工作集约 162.08/306.08 ns。频率不同，所以 ns 更适合比较墙钟延迟。*

![图 32：A72 与 Kryo 的 4 KB 页延迟周期](arm_cortex_a72_figures/32_a72_kryo_latency_cycles.jpg)
...
![图 33：A72 与 Kryo 单核 Read 带宽，GB/s](arm_cortex_a72_figures/33_single_core_read_bandwidth_gbps.jpg)

*图 33：A72/Kryo 的 L1 峰值约 36.65/40.15 GB/s；A72 在 16 KB 后先跌至约 23，再回到约 26，L2 范围约 19.52；Kryo L2 约 17.78。进入内存后两者约 10 GB/s。*

![图 34：单核 Read 带宽换算为 B/cycle](arm_cortex_a72_figures/34_single_core_read_bandwidth_per_cycle.jpg)
...
![图 35：单核 Write 带宽](arm_cortex_a72_figures/35_single_core_write_bandwidth.jpg)
...
![图 36：Graviton 1 四核 Cluster 的 Read 带宽](arm_cortex_a72_figures/36_cluster_read_scaling.jpg)
...
![图 37：Graviton 1 四核 Cluster 的 Write 带宽](arm_cortex_a72_figures/37_cluster_write_scaling.jpg)
...
![图 38：共享末级 Cache Read 扩展](arm_cortex_a72_figures/38_l2_read_haswell_comparison.jpg)
...
![图 39：按频率归一后的共享 Cache Read](arm_cortex_a72_figures/39_l2_read_clock_normalized.jpg)
...
![图 40：共享末级 Cache Write 扩展](arm_cortex_a72_figures/40_l2_write_haswell_comparison.jpg)
...
![图 41：按频率归一后的共享 Cache Write](arm_cortex_a72_figures/41_l2_write_clock_normalized.jpg)
...
![图 42：Graviton 1 的 DRAM Read 扩展](arm_cortex_a72_figures/42_dram_bandwidth_scaling.jpg)
...
![图 43：Graviton 1 核间延迟矩阵](arm_cortex_a72_figures/43_graviton_core_latency.png)

*图 43：单位 ns。四核 Cluster 内约 55 ns，跨 Cluster 约 217～220 ns，边界非常清晰。网页没有披露锁/共享线状态与同步协议，因此矩阵主要用于识别拓扑。*

![图 44：Haswell 核间延迟](arm_cortex_a72_figures/44_haswell_core_latency.png)

*图 44：同物理核的 SMT 线程约 6.4～6.7 ns，跨核通常约 18～22 ns。高频 Ring 与 L3 紧耦合，让它远低于 Graviton。*

![图 45：Snapdragon 821 Kryo 核间延迟](arm_cortex_a72_figures/45_kryo_core_latency.png)
