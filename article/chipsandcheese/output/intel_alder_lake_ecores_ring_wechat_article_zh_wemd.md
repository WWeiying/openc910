...
themeName: "学术论文 (副本)"
title: "intel_alder_lake_ecores_ring_wechat_article_zh"
---
...
![图 1：E-Core 空闲与运行 NOP 时，P-Core 侧缓存和内存延迟对比](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_ecores_ring_wechat_article_zh/4e29ac7c6c16b182_01_figure.png)

图 1 显示，在 L3 容量范围内，ring 降频带来约 11.7% 的延迟惩罚，绝对差约 1.78 ns。进入内存后，差距扩大到 3.4 ns，但相对增幅降至 3.7%，因为 DRAM 自身延迟占据了更大比例。

![图 2：把延迟换算为 P-Core 周期后的差异](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_ecores_ring_wechat_article_zh/335d64d5c18684e0_02_figure.jpg)

按核心周期看，激活 E-Core 让 P-Core 的共享层级访问多出约 9—10 周期。Golden Cove 虽有很大的乱序窗口，但 demand L3 hit 依然更难完全隐藏，因而更依赖大容量私有缓存和预取器维持指令吞吐。

## 带宽：L3 下降约 20%，内存影响很小

![图 3：单个 P-Core 侧的 L3 与内存带宽](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_ecores_ring_wechat_article_zh/7f8b30c6df6e4b4c_03_figure.jpg)

单核测试里，L3 带宽下降约 20%，内存带宽只下降约 3.2%。由于假负载没有向 L3 发请求，这个差异不能归因于 E-Core 与 P-Core 争抢缓存带宽；ring 的频率变化才是主要变量。

![图 4：全部 P-Core、每核单线程时的聚合带宽](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_ecores_ring_wechat_article_zh/718b43694df8d17d_04_figure.jpg)
...
![图 5：P-Core 压缩负载在两种 ring 状态下的性能](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_ecores_ring_wechat_article_zh/38287fd0934cccb7_05_figure.png)

E-Core 保持空闲时，压缩性能提高 2.9%。这与 L3 延迟和带宽改善方向一致，但单项应用无法把全部差异精确归因到某一级缓存。

![图 6：P-Core 编码负载在两种 ring 状态下的性能](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_ecores_ring_wechat_article_zh/dc80dd7f0a9657e0_06_figure.png)
