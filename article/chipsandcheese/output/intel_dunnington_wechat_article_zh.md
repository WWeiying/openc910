...
![图 1：四路 Dunnington 服务器的大量 FBDIMM](intel_dunnington_figures/01_figure.jpg)
...
![图 2：Dunnington 裸片，中央 CBC、三组 Penryn 与不规则 16 MB L3](intel_dunnington_figures/02_figure.jpg)
...
![图 3：Penryn 核心框图](intel_dunnington_figures/03_figure.png)
...
![图 4：Penryn 的 taken branch footprint 与延迟](intel_dunnington_figures/04_figure.png)

![图 5：Penryn、AMD K8/K10 的连续 taken branch 能力](intel_dunnington_figures/05_figure.png)
...
![图 6：L1I/L2/DRAM 不同代码 footprint 下的 IPC](intel_dunnington_figures/06_figure.png)
...
![图 7：Penryn、NetBurst 与 K10 的在途结构容量](intel_dunnington_figures/07_figure.jpg)
...
![图 8：Penryn 与 K10 执行端口和主要操作吞吐/延迟](intel_dunnington_figures/08_figure.jpg)
...
![图 9：Merom/Penryn 与 K10 的地址生成组织](intel_dunnington_figures/09_figure.png)

地址完全相同且 load 不跨 64 B line 时，Penryn store forwarding 约 5 周期；若 store 64-bit 对齐，也能转发其上半部分。

![图 10：Penryn 的 store-to-load forwarding 矩阵](intel_dunnington_figures/10_figure.png)
...
![图 11：Penryn 与 NetBurst 的转发失败对比](intel_dunnington_figures/11_figure.png)

![图 12：K10 转发矩阵](intel_dunnington_figures/12_figure.png)
...
![图 13：Dunnington 测试系统缓存拓扑，来自 lstopo](intel_dunnington_figures/13_figure.png)

### 32 KB L1D

32 KB、8-way、3 周期，8 bank 支持每周期一 load+一 store；128-bit load/store 各只需一次 L1D access。

![图 14：Penryn L1D 延迟、容量与带宽](intel_dunnington_figures/14_figure.png)
...
![图 15：Intel 论文中的 Penryn L2 组织](intel_dunnington_figures/15_figure.png)
...
![图 17：单核和双核共享 Penryn L2 带宽](intel_dunnington_figures/17_figure.png)
...
![图 18：Tulsa Hot Chips 框图；Dunnington 复用类似 uncore 概念](intel_dunnington_figures/18_figure.png)

![图 19：Intel Technology Journal 的 Dunnington 高层架构](intel_dunnington_figures/19_figure.png)
...
![图 20：Dunnington L3 容量/相联度与延迟](intel_dunnington_figures/20_figure.png)

L3 约 37 ns，比 AMD 高；Tulsa 官方约 35 ns，与继承关系一致。大容量可解释一部分延迟，现代 Ampere Altra 也约 35 ns，但这仍属于服务器中的偏高水平。

![图 21：Dunnington L3 单核、module 与全 socket 带宽](intel_dunnington_figures/21_figure.png)
...
![图 24：Intel 参考 MCH 散热器，使用 PCM45F thermal pad](intel_dunnington_figures/24_figure.jpg)
...
![图 25：同核、同 module、跨 module 的 cacheline bounce 延迟](intel_dunnington_figures/25_figure.png)
...
![图 28：同期 Opteron 的同 socket/跨 socket cacheline bounce](intel_dunnington_figures/28_figure.png)
...
![图 29：Dunnington 与 K10 DRAM 聚合带宽](intel_dunnington_figures/29_figure.png)
...
![图 30：Paxville 两颗 NetBurst die 直接共享一条 FSB 的封装布线](intel_dunnington_figures/30_figure.jpg)

Paxville 已在部分负载遇到高 FSB utilization，Tulsa 先加 uncore，Dunnington 再换成 Penryn module。

![图 31：Dunnington 封装布线，FSB 接口位于 die 中央](intel_dunnington_figures/31_figure.jpg)
