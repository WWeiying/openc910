...
![图 1：MLiD 展示的 Zen 5“微架构亮点”页面，真实性在文章发表时无法确认](amd_zen5_leaked_slides_figures/01_figure.jpg)
...
![图 2：A64FX 预测/取指流水线，展示快速小型 predictor 与较慢 BTB 路径](amd_zen5_leaked_slides_figures/02_figure.png)
...
![图 3：Zen 4 上 conditional always-taken 与 unconditional branch 的吞吐差异很小](amd_zen5_leaked_slides_figures/03_figure.png)
...
![图 4：Zen 4、Golden Cove 等核心的 branch target tracking；Golden Cove 最后一级 BTB 容量约高 50%](amd_zen5_leaked_slides_figures/04_figure.png)
...
![图 5：三个基本块的假想代码布局](amd_zen5_leaked_slides_figures/05_figure.png)

最普通、也最可能的解释是：Zen 5 能跨 not-taken branch 继续取指。过去二十年的高性能 AMD/Intel core 本来就能做到。

![图 6：顺序布局且 branch not taken 时，一个 fetch 可跨越两个 basic block](amd_zen5_leaked_slides_figures/06_figure.png)
...
![图 7：跨 taken branch 取两个 block 需要多目标预测、双取指路径和下游 merge](amd_zen5_leaked_slides_figures/07_figure.png)
...
![图 8：x86-64 四级页表以及一次 page walk 的依赖访问路径](amd_zen5_leaked_slides_figures/08_figure.png)
...
![图 9：不同 PWC 组织可在“覆盖更大地址范围”与“少走更多层”之间取舍](amd_zen5_leaked_slides_figures/09_figure.png)
...
![图 10：AMD Zen 3 Hot Chips 图，IPC 增长主要来自 cache、frontend、branch predictor 与 load/store](amd_zen5_leaked_slides_figures/10_figure.png)
...
![图 11：Samsung Exynos 论文中的 IPC 分布，只有少数 workload 接近宽度上限](amd_zen5_leaked_slides_figures/11_figure.png)
...
![图 12：Henry Wong 博士论文中的 distributed scheduler 设计空间](amd_zen5_leaked_slides_figures/12_figure.png)

unified scheduler 让多个 port 共享 entry，更能吸收某类操作突增，却必须每周期完成多路选择。近期 AMD、Intel、Arm 都采用混合方案。

![图 13：Zen 1、Zen 2 与 Zen 3/4 的分布式/部分统一 scheduler 演进](amd_zen5_leaked_slides_figures/13_figure.png)

泄露页称 Zen 5 scheduler 更大、更 unified，意味着 entry 增加，也能更灵活使用。对两个游戏的观察显示 integer scheduler 0 略比其他队列常满。

![图 14：两个游戏中触发 rename/dispatch stall 的资源分布](amd_zen5_leaked_slides_figures/14_figure.png)

Cinebench 2024 整数侧也相似。scheduler 0 服务一条 AGU 和一条 ALU/branch pipe，AMD 可扩它、与另一队列合并，或两者兼用。

![图 15：Cinebench 2024 的 scheduler-related dispatch stall，integer scheduler 0 占比最高](amd_zen5_leaked_slides_figures/15_figure.png)
...
![图 16：一种低增量成本的 6 ALU、4 AGU 假想布局，AGU 与 branch port 兼做简单 ALU](amd_zen5_leaked_slides_figures/16_figure.png)
...
![图 17：Cinebench 2024 中 Zen 4 的 dispatch stall，ROB full 是主要来源之一](amd_zen5_leaked_slides_figures/17_figure.png)

ROB 跟踪后端全部 instruction，限制核心能越过 stalled instruction 看多远。ROB fill 并非设计失败，它也说明更专用的 queue 足够大，没有更早成为瓶颈。

![图 18：AMD 各代 ROB 容量，从 Phenom 72、Piledriver 128 到 Zen 4 320 项](amd_zen5_leaked_slides_figures/18_figure.png)
...
![图 19：Zen 3/4 的 L1 与 L2 data prefetcher，包括 stream、stride、region 与 up/down 模式](amd_zen5_leaked_slides_figures/19_figure.png)
...
![图 20：AMD ISSCC 对 Zen 4“power-efficient AVX-512”的说明，512-bit 操作由 256-bit datapath 执行](amd_zen5_leaked_slides_figures/20_figure.png)
...
![图 21：AMD 官方 Zen 4 desktop IPC 构成，约 13% 增长来自多个前后端小改进](amd_zen5_leaked_slides_figures/21_figure.png)

但不能把当前 leak 看得太深。细节稀少给了解释很大空间，核心行为还可由 microcode 调整，架构本身也可能配置化；Zen 2 就有不同 FPU 配置。

![图 22：MLiD 展示的另一张 x86 roadmap，暗示不同 FP-512 variant；仍属未核验材料](amd_zen5_leaked_slides_figures/22_figure.jpg)

性能数字更应先当猜测。即使消息源是工程师，一个特定 trace 在 simulation 中提升 30% IPC，也不代表其他应用得到同样幅度。

![图 23：AMD 官方历代 desktop IPC 图，Zen 2/3/4 的典型代际增幅约 10%—20%](amd_zen5_leaked_slides_figures/23_figure.jpg)

中间信息经转述后，很容易丢掉“仅对一条 trace”的测试条件。

![图 24：Red Gaming Tech 视频中的 Zen 5 传闻汇总，声称 20%—30% IPC 等多项提升](amd_zen5_leaked_slides_figures/24_figure.jpg)
