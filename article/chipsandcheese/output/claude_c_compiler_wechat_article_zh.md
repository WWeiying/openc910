...
![图 1：指针追踪式 C 循环，前一次数组读取结果决定下一次下标](claude_c_compiler_figures/01_figure.png)

`current = A[current]` 建立严格的数据依赖，乱序核心也无法让多次读取并行；总时间除以迭代次数，就得到单次访问延迟。CCC 与 GCC 生成的汇编差异很大。

![图 2：同一循环在 x86-64 与 AArch64 上由 CCC、GCC 生成的汇编](claude_c_compiler_figures/02_figure.png)

GCC 用 indexed addressing 把移位、相加和 load 表达在一条 x86 指令中。CCC 则把访问拆成更多简单指令，看起来更符合文章戏仿的“纯粹 RISC”路线。

![图 3：x86-64、最高优化级别下，CCC 与 GCC 的依赖数据流](claude_c_compiler_figures/03_figure.png)
...
![图 4：AArch64、最高优化级别下的依赖数据流](claude_c_compiler_figures/04_figure.png)
...
![图 5：不同核心上 GCC 与 CCC 版本测得的一级缓存依赖访问延迟](claude_c_compiler_figures/05_figure.png)

窄执行引擎的小核心更难吸收 loop overhead 与 `sum += current` 的附加代码，顺序执行核心的损失最大。

![图 6：CCC 为 AArch64 循环计数与汇总语句生成的冗长数据流，甚至包含寄存器自搬运](claude_c_compiler_figures/06_figure.png)
...
![图 7：故意包含多处不兼容类型操作的 C 程序](claude_c_compiler_figures/07_figure.png)
...
![图 8：GCC 的类型错误信息与 CCC 仍然产出可执行文件的命令行结果](claude_c_compiler_figures/08_figure.png)
...
![图 9：八个 C workload 在三种核心上的估算 SPEC CPU2017 分数](claude_c_compiler_figures/09_figure.png)
...
![图 10：CCC 相对 GCC 的性能回退比例](claude_c_compiler_figures/10_figure.png)
...
![图 11：CCC 与 GCC 版本的平均 IPC](claude_c_compiler_figures/11_figure.png)
...
![图 12：八个 workload 的退休指令总数；CCC 在部分负载上执行超过十倍指令](claude_c_compiler_figures/12_figure.png)
...
![图 13：Ryzen 9800X3D 上 CCC 与 GCC 代码的 Top-Down 构成](claude_c_compiler_figures/13_figure.jpg)
...
![图 14：Zen 5 上 CCC/GCC 版本的 op cache coverage，500.perlbench 差异最大](claude_c_compiler_figures/14_figure.png)

![图 15：Lion Cove 上 CCC 与 GCC 版本的 Top-Down 构成](claude_c_compiler_figures/15_figure.jpg)
...
![图 16：Cortex-X925 上 CCC 与 GCC 版本的 Top-Down 构成](claude_c_compiler_figures/16_figure.jpg)
...
![图 17：CCC 版 519.lbm 的热点汇编，反复把 x0 写入 stack，数条指令后又加载回来](claude_c_compiler_figures/17_figure.jpg)
...
![图 18：CCC 版 Lion Cove 与 GCC 版 AMD FX-8150 的估算 SPEC 分数对比](claude_c_compiler_figures/18_figure.png)
