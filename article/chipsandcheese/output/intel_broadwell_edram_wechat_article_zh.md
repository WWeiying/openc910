...
![图 1：Broadwell 桌面产品与裸片背景](intel_broadwell_edram_figures/01_figure.jpg)
...
![图 2：Intel ISSCC 论文中的 Crystal Well eDRAM 裸片](intel_broadwell_edram_figures/02_figure.jpg)

eDRAM（embedded DRAM）每 bit 只需一个晶体管和电容，密度优于用多个晶体管锁存一 bit 的 SRAM。IBM POWER 也曾用 eDRAM 做 L3。

![图 3：22 nm eDRAM 与更新 14 nm SRAM 的 bit density 对比](intel_broadwell_edram_figures/03_figure.jpg)
...
![图 4：Crystal Well 的阵列与 bank 组织](intel_broadwell_edram_figures/04_figure.jpg)
...
![图 5：四象限 banking；每象限选一个 bank，两周期各读出 16 B，共组成 64 B cache line](intel_broadwell_edram_figures/05_figure.jpg)

每个 bank 数据宽度为 64 bit（8 B），图 5 展示一条 64 B cache line 如何由四个象限并行供给。

### 分离读写总线的 OPIO

![图 6：Haswell/Broadwell 相同的 OPIO 双向独立 64-bit 数据总线](intel_broadwell_edram_figures/06_figure.jpg)
...
![图 7：Crystal Well 自刷新与功耗状态](intel_broadwell_edram_figures/07_figure.jpg)
...
![图 8：L3 slice 中并行检查 L3 与 eDRAM tag](intel_broadwell_edram_figures/08_figure.png)

L3 与 L4 tag 并行查，L4 miss 不必先绕 ring 访问 eDRAM 控制器、再回头访问内存。

![图 9：eDRAM 控制器拥有独立 ring stop](intel_broadwell_edram_figures/09_figure.png)
...
![图 10：Broadwell L1/L2/L3/L4/DRAM load-to-use 延迟](intel_broadwell_edram_figures/10_figure.jpg)

L4 约 36.6 ns，在 3.8 GHz 接近 140 周期，远不能替代 L3；6 MB L3 仍负责低延迟命中。

![图 11：Broadwell eDRAM 与无 eDRAM Haswell 的延迟](intel_broadwell_edram_figures/11_figure.png)
...
![图 12：eDRAM 只读与 read-modify-write 带宽](intel_broadwell_edram_figures/12_figure.png)

只读约 50 GB/s 封顶；读改写因为双向总线而更高，但未翻倍，可能受高负载 bank conflict 限制。这是根据现象的解释，不是计数器确认。

![图 13：L3 与 eDRAM 带宽随核心数扩展](intel_broadwell_edram_figures/13_figure.png)
...
![图 14：Intel 对 Haswell 有/无 eDRAM 的带宽利用率—延迟测试](intel_broadwell_edram_figures/14_figure.jpg)

复测时启动可变数量的带宽线程，并用 busy-loop 节流；另一个延迟线程固定在独立核心，使用 2 MB 大页降低地址翻译开销。

![图 15：Broadwell 复测；32+64 MB 命中 eDRAM，1+1 GB 进入 DRAM](intel_broadwell_edram_figures/15_figure.png)
...
![图 16：CBo、ring、eDRAM 与 System Agent ARB 的关系](intel_broadwell_edram_figures/16_figure.png)

![图 17：不同工作集下 CBo L3 miss 与 ARB 请求的关系](intel_broadwell_edram_figures/17_figure.png)
...
![图 18：SPEC CPU2017 中按 CBo/ARB 推算的 L4 命中行为](intel_broadwell_edram_figures/18_figure.png)

因此后面的“L4 hitrate”只是估算，不是精确硬件事件。

![图 19：SPEC CPU2017 单项结果与估算 L4 命中率](intel_broadwell_edram_figures/19_figure.png)

`520.omnetpp` 的 L3 miss 很多，估算 L4 hitrate 超过 84%，足以让 Broadwell 超过频率更高、架构更新的 Skylake。

![图 20：Broadwell 与 Skylake 的 SPEC CPU2017 汇总](intel_broadwell_edram_figures/20_figure.png)
...
![图 21：Broadwell 与 Skylake 的 libx264 性能](intel_broadwell_edram_figures/21_figure.png)

向量密集编码中 Skylake 领先 4.8%。

![图 22：libx264 的 System Agent 流量](intel_broadwell_edram_figures/22_figure.png)
...
![图 23：Broadwell 与 Skylake eDRAM 系统组织对比](intel_broadwell_edram_figures/23_figure.png)

所有主存访问都可受益，display engine 尤其合适：阅读网页或编辑文档时 CPU 很闲，屏幕刷新却持续占用 DRAM 带宽。

![图 24：Skylake eDRAM 与主存带宽](intel_broadwell_edram_figures/24_figure.png)

带宽略有改善，可能小幅提高了 eDRAM/OPIO 频率。但 tag 不再与 L3 并行查；L3 miss 到 System Agent 后才知道是否命中 L4。主存又必须等 L4 tag 判 miss 才能启动，否则会失去缓存意义。

![图 25：Skylake eDRAM 和 DRAM 的 load-to-use 延迟](intel_broadwell_edram_figures/25_figure.png)

L4 远离 CPU、靠近内存控制器，CPU 这种延迟敏感客户端吃亏；Skylake eDRAM 延迟已接近快速 DDR3，DRAM 延迟也因串行 tag 检查而回退。

![图 26：eDRAM、DDR4 与 DDR5 的带宽/延迟演进](intel_broadwell_edram_figures/26_figure.png)
...
![图 27：Crystal Well 与主存 DRAM 设计取舍](intel_broadwell_edram_figures/27_figure.png)

Crystal Well 证明，主存 DRAM 的少 bank、双向共享总线和较长 bank cycle 是成本取舍，不是 DRAM 技术不可改变的天条。

![图 28：IBM POWER8 片上 eDRAM L3](intel_broadwell_edram_figures/28_figure.jpg)
...
![图 29：Broadwell eDRAM 与 AMD 3D V-Cache 的延迟/容量定位](intel_broadwell_edram_figures/29_figure.png)
