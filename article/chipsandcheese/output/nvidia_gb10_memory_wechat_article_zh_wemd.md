...
themeName: "学术论文 (副本)"
title: "nvidia_gb10_memory_wechat_article_zh"
---
...
![图 1：GB10 与 Strix Halo 的 CPU/内存层次对照](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/a3df7d3a6868392d_01_figure.png)
...
![图 2：A725 看到的 Cache 与 DRAM 延迟](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/a677bdf0be10551d_02_figure.png)

*图 2：A725 的 L2 约 3.2 ns，越过 512 KB 后进入超过 21 ns、60 cycle 以上的 L3 区间。两簇 L3 容量不同，但 A725 看到的 L3 延迟接近。*

![图 3：X925 看到的 Cache 与 DRAM 延迟](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/da1f9228dc777d8a_03_figure.png)
...
![图 4：GB10、Strix Halo 的主要容量与延迟](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/ec3314fa3052a539_04_figure.png)

*图 4：表中汇总 A725、X925 与 Zen 5。GB10 的核心私有 Cache 以 cycle 计并不慢，但 Zen 5 频率更高，折算成纳秒后仍更快；AMD 的 32 MB L3 还兼具更低延迟。*

![图 5：三类核心的完整 Cache/DRAM 曲线](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/bb341537d2c2e7ca_05_figure.png)
...
![图 6：单线程读取带宽随工作集变化](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/61c19b475a4c386f_06_figure.png)
...
![图 7：簇 0 的逐线程私有数组带宽](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/85e98410cf199b39_07_figure.png)

*图 7：工作集跨出私有 L2 后，曲线开始反映 8 MB L3 与簇外接口。每线程独立数组更接近多个互不相同工作集的真实负载。*

![图 8：簇 1 的逐线程私有数组带宽](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/38778390c4170fa4_08_figure.png)
...
![图 9：GB10 两簇与 Strix Halo 的簇级带宽](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/bcb78ae7dc483010_09_figure.png)

*图 9：纯读时簇 0/1 约 63.1/115.5 GB/s，两簇合计约 139 GB/s；1:1 读写时约 108.9/150/153 GB/s。读写混合显著提高总量，支持独立读写路径的解释。*

Arm DSU-120 最多可配置四条 256-bit CHI 接口。簇 1 读取超过 100 GB/s，而簇 0 更窄，可能来自接口数量不同；这仍是基于带宽的推测。奇特之处在于，两个簇却都保留相同的五大五小异构构成。

![图 10：DGX Spark 的 GB10 资料与系统识别](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/bc189acba52546c4_10_figure.jpg)
...
![图 11：先加入 A725 时的簇内受载延迟](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/d9eb2d6f526d2378_11_figure.png)

*图 11：横轴是带宽线程数，蓝线为带宽、橙线为延迟。两簇均显示核心类型比单纯线程数更能决定排队行为。*

![图 12：先加入 X925 时的簇内受载延迟](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/e575b34625ee6352_12_figure.png)
...
![图 13：GB10 两簇与 Strix Halo 的 CPU 受载延迟](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/f2c6f0a567763abf_13_figure.png)

*图 13：曲线比较的是不同系统和可达带宽区间。GB10 的低基线很有价值，但接近极限时仍会因队列堆积而快速抬升。*

GPU 加压会把问题放大。只让 GPU 制造带宽，CPU 延迟在 GPU 达到 231 GB/s 时已超过 351 ns。

![图 14：GPU 带宽压力下的 CPU 延迟](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/eaaf165c3aab0d93_14_figure.png)
...
![图 15：CPU 与 GPU 同时加压的带宽和延迟](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/36f520e782affe5a_15_figure.png)
...
![图 16：GB10 的 20 核核间延迟矩阵](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/f8f5bf28164ba714_16_figure.jpg)

*图 16：对角线附近的绿色块对应簇内传输，跨簇区域明显变成橙红色。矩阵中的大核/小核位置也呈现差别。*

![图 17：按簇内与跨簇重新着色的矩阵](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/df9a8d4581b449c4_17_figure.jpg)
...
![图 18：GB10 与 Strix Halo 核间延迟对照](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/nvidia_gb10_memory_wechat_article_zh/ffcfa9951a9bd425_18_figure.png)
