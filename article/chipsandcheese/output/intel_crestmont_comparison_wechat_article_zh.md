...
![图 1：Intel Tech Tour 展示标准 E-Core 与 SoC Tile LPE-Core](intel_crestmont_comparison_figures/01_figure.jpg)
...
![图 2：libx264 的 Top-Down slot 分类](intel_crestmont_comparison_figures/02_figure.png)

两种 Crestmont 都以 backend bound 为主。

![图 3：后端结构满导致的 stall 细分](intel_crestmont_comparison_figures/03_figure.png)

Load buffer 最热，说明大量 load 等待更老操作；ROB 也频繁填满，核心在努力隐藏长延迟；依赖这些慢指令的 uop 继续占 scheduler。更大 buffer/scheduler 会有帮助，但先要看延迟来源。

![图 4：L1D miss 后分别等待 L2/L3/DRAM 的周期占比](intel_crestmont_comparison_figures/04_figure.png)

L2 虽不算快，现代乱序核仍可隐藏二十多周期，两种核心只在 2%—4% 周期等待 L2。标准 Crestmont 面对较高 L3 延迟也只有 6.31%。DRAM 则是数百周期，LPE-Core 无法隐藏；Redwood Cove 大核也只是减轻。

![图 5：每指令 DRAM access 与 DRAM-bound cycle](intel_crestmont_comparison_figures/05_figure.png)
...
![图 6：Kernel compile Top-Down 分类](intel_crestmont_comparison_figures/06_figure.png)

![图 7：Frontend latency 与 bandwidth 损失](intel_crestmont_comparison_figures/07_figure.png)
...
![图 8：Crestmont 双 decode cluster 与 toggle 机制示意](intel_crestmont_comparison_figures/08_figure.jpg)

大量 branch 还会访问更大、更慢 BTB level，decoder 有时发现 predictor 未跟踪的 taken branch，Crestmont 可能受益于更大 BTB。

![图 9：指令 cache miss 造成的前端 stall](intel_crestmont_comparison_figures/09_figure.png)
...
![图 10：Kernel compile 后端结构压力](intel_crestmont_comparison_figures/10_figure.png)

![图 11：数据侧 L2/L3/DRAM stall](intel_crestmont_comparison_figures/11_figure.png)
...
![图 12：7-Zip Top-Down 分类](intel_crestmont_comparison_figures/12_figure.png)

![图 13：Bad speculation 几乎全部来自 branch mispredict](intel_crestmont_comparison_figures/13_figure.png)

Memory ordering violation 或错误 load independence 也可浪费执行，但此处贡献很小。

![图 14：三种负载的 branch prediction accuracy](intel_crestmont_comparison_figures/14_figure.png)

7-Zip 准确率 95.42%，比早期 Atom 好很多，只略低于另两项。

![图 15：按每指令 mispredict 统计，7-Zip 负担显著](intel_crestmont_comparison_figures/15_figure.png)

问题是 branch 占指令流超过 17%；即使 95.42%，每千指令仍会错很多，恢复丢失大量工作。

![图 16：7-Zip 后端结构压力](intel_crestmont_comparison_figures/16_figure.png)

Scheduler 经常满。7-Zip 几乎没有 FP/vector，因此分布式 integer scheduler 是主要瓶颈；大量指令互相依赖，ILP 难找，与 kernel compile 的 ROB-first 不同。

![图 17：两种 Crestmont 的 cache miss stall](intel_crestmont_comparison_figures/17_figure.png)

两者 cache 痛感接近：L1D hitrate 高，大多数 miss 又被 2 MB L2 捕获。

![图 18：标准 Crestmont 24 MB L3 在 7-Zip 中仅约 35% hitrate](intel_crestmont_comparison_figures/18_figure.png)
...
![图 19：Crestmont/Gracemont 核心图；来源分别为 Bilibili 页面与 Fritzchens Fritz](intel_crestmont_comparison_figures/19_figure.jpg)

![图 20：Intel Tech Tour 展示 E-Core 处理轻量任务](intel_crestmont_comparison_figures/20_figure.jpg)
