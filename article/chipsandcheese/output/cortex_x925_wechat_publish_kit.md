...
- 母稿：`cortex_x925_wechat_article_zh.md`
- WeMD 发布副本：`cortex_x925_wechat_article_zh_wemd.md`
- WeMD 主题：`custom-1786280678341-jnfpaqasm`（学术论文（副本））

发布时将 WeMD 副本导入 WeMD。该副本由母稿自动生成，不单独修改；母稿有更新时，重新运行生成脚本即可同步正文和图片。
...
- WeMD 副本中的 25 张图片均已上传至腾讯云 COS，匿名读取、HTTPS、图片类型和 `MAZ_STANDARD` 存储类型均已验证

图片按文件名前缀顺序上传。公众号编辑器中保留每张图片下方的斜体图注。图 6、7、11、18、20～25 含密集数字，上传时不要二次裁切；图 16、17 是高密度 forwarding 矩阵，应使用原尺寸文件并允许读者点开查看。

当前图片使用 COS 默认域名。对象可以正常作为图片资源读取，但直接打开时腾讯云会返回 `Content-Disposition: attachment` 和 `x-cos-force-download: true`。正式发布前优先切换到已配置 HTTPS 的自定义域名，并重新生成 WeMD 副本；这样可避免默认域名的强制下载策略影响公众号抓图或读者查看原图。
