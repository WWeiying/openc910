...
![图 1：Knights Landing 与 Silvermont 核心结构对比](intel_knights_landing_figures/01_figure.png)

## 前端：小预测器靠 SMT 隐藏延迟

KNL 难以识别长分支模式，低于同期 Cortex-A72。

![图 2：分支模式长度测试](intel_knights_landing_figures/02_figure.png)

![图 3：KNL 与其他低功耗/桌面核心方向预测能力](intel_knights_landing_figures/03_figure.jpg)
...
![图 4：KNL taken branch footprint](intel_knights_landing_figures/04_figure.png)
...
![图 5：KNL 与 Cortex-A72 的分层 taken branch 延迟](intel_knights_landing_figures/05_figure.png)

Return stack 16 项，SMT 静态分区：1 thread 得 16，2 thread 各 8，3/4 thread 各 4；三线程时无法均分，部分闲置。

![图 6：不同 SMT 活动数下 RAS 深度](intel_knights_landing_figures/06_figure.png)

### 应用中的预测

7-Zip branch rate 高但代码 footprint 小，甚至能放进现代核心 uop cache；KNL 四线程时仍可能表现合理。

![图 7：7-Zip 随 SMT 数变化的 branch MPKI](intel_knights_landing_figures/07_figure.png)

单线程下它好于 Phytium，说明有限资源调得不错，但不敌桌面大预测器。

![图 8：单线程预测器跨架构对比](intel_knights_landing_figures/08_figure.png)

libx264 branch rate 更低、代码 footprint 更大，增加 SMT thread 后预测准确率却剧跌。

![图 9：libx264 随 SMT 数变化的 branch MPKI](intel_knights_landing_figures/09_figure.png)

Intel BAClear 表示预测单元没提供正确目标，decoder 重新算 target 并轻度 flush，可近似观察 BTB miss。

![图 10：7-Zip/libx264 的 BAClear；libx264 随 SMT thread 激增](intel_knights_landing_figures/10_figure.png)

POWER9 名义 SMT8，更适合理解为两个资源丰富的 SMT4 group，四线程下预测没有崩坏。

![图 11：POWER9 与 KNL 四线程 branch prediction](intel_knights_landing_figures/11_figure.png)
...
![图 12：不同代码 footprint 的取指吞吐；KNL flat DDR4，MCDRAM 结果类似](intel_knights_landing_figures/12_figure.png)

四线程也不能提高每核 L2 code bandwidth，4-byte 指令仍低于 1 IPC，提示 instruction-side MLP 很低。

![图 13：不同指令长度下的 L2 取指 IPC](intel_knights_landing_figures/13_figure.png)

![图 14：KNL、Bobcat、Cortex-A72 与大核的大 footprint 对比](intel_knights_landing_figures/14_figure.png)

HPC loop 通常小，问题不大；客户端应用代码大，多线程进一步削弱 locality。

![图 15：SMT 对实际前端供给的改善](intel_knights_landing_figures/15_figure.png)
...
![图 16：KNL 的 ROB、整数/向量/mask rename 等容量](intel_knights_landing_figures/16_figure.jpg)
...
![图 17：其他 SMT thread 运行 spinloop 时的结构容量；三与四线程相同](intel_knights_landing_figures/17_figure.png)

SMT4 每线程只有约 18 ROB，所有 rename 类型覆盖这 18 项反而必要。Store queue 更紧，每线程仅 4 项。

![图 18：随 SMT 数变化的 load/store queue 可用容量](intel_knights_landing_figures/18_figure.png)
...
![图 19：裸片上的 AVX-512 执行区，标注为推测](intel_knights_landing_figures/19_figure.jpg)

每周期 FMA 吞吐可匹敌 Skylake-X，也能两条 512-bit integer add；延迟更长。

![图 20：KNL 与 Skylake-X 向量吞吐/延迟；KNL 对较窄向量也近似](intel_knights_landing_figures/20_figure.jpg)
...
![图 21：Henry Wong 方法测得的 KNL store-to-load forwarding](intel_knights_landing_figures/21_figure.png)
...
![图 22：KNL L1D/L2 延迟与容量](intel_knights_landing_figures/22_figure.png)
...
![图 23：开盖 KNL 封装内的 MCDRAM die](intel_knights_landing_figures/23_figure.jpg)
...
![图 24：IEEE 论文中的 MCDRAM cache 实现](intel_knights_landing_figures/24_figure.jpg)
...
![图 25：Flat、Cache 与 DDR4 的 load-to-use latency](intel_knights_landing_figures/25_figure.png)
...
![图 26：页面大小与 TLB reach 造成的延迟台阶](intel_knights_landing_figures/26_figure.png)
...
![图 27：单 tile L1/L2/DRAM 带宽](intel_knights_landing_figures/27_figure.png)

L2 理论 64 B/cycle；单核略低，双核接近。单核加载全部 SMT thread，DRAM 约 13 GB/s；同 tile 第二核提升不大。

![图 28：全芯片 DDR4、MCDRAM flat/cache 的带宽](intel_knights_landing_figures/28_figure.png)
...
![图 29：Hybrid+SNC4 各 NUMA node 的容量/带宽曲线](intel_knights_landing_figures/29_figure.png)
...
![图 30：本地与远端 MCDRAM 的延迟](intel_knights_landing_figures/30_figure.png)

Local controller 从约 179 降至 170 ns，仍比 DDR4 慢，证明 MCDRAM 优先带宽而非延迟。

![图 31：SNC4 中本地/远端 DDR4 与 MCDRAM；结果不同于典型 NUMA](intel_knights_landing_figures/31_figure.png)

Flat+quadrant+SNC4 的本地 DDR4 最低约 143 ns。Remote node 带宽几乎接近 local，延迟罚也轻；单片 die 的密集 mesh 让 NUMA 特征温和。

![图 32：随启用核心数增加的 DDR4/MCDRAM 带宽](intel_knights_landing_figures/32_figure.png)
...
![图 33：全芯片 core-to-core latency 矩阵；Linux 编号转换为 Windows 习惯](intel_knights_landing_figures/33_figure.png)

![图 34：quadrant+SNC4 的核间矩阵](intel_knights_landing_figures/34_figure.png)
...
![图 35：7-Zip/libx264 等应用随 SMT thread 数的绝对性能](intel_knights_landing_figures/35_figure.png)

![图 36：相对每核单线程的 SMT scaling](intel_knights_landing_figures/36_figure.png)

只要不受带宽限制，SMT4 增益可观。

![图 37：随 SMT 增加的每指令 offcore response](intel_knights_landing_figures/37_figure.png)
...
![图 39：Fritzchens Fritz 的 KNL 裸片](intel_knights_landing_figures/39_figure.jpg)

每核心约 2.93 mm²，显著小于同 14 nm Skylake；vector execution 约 1.14 mm²、占 39%；L1D 为吞吐牺牲密度，占核心略超 10%。

![图 40：KNL tile/core 推测标注](intel_knights_landing_figures/40_figure.jpg)

![图 41：C2000 Silvermont 裸片](intel_knights_landing_figures/41_figure.jpg)
...
![图 42：从 idle 到 1.3 GHz 的升频轨迹](intel_knights_landing_figures/42_figure.png)
