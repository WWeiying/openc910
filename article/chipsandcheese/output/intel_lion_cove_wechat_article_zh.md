# Lion Cove：Intel P-Core 再度咆哮，八宽核心与多级 Cache
...
![图 1：Lion Cove 所处的代际位置](intel_lion_cove_figures/01_lion_cove_generation_overview.jpg)
...
![图 2：Lunar Lake 的 Ring Bus 与 L3 组织](intel_lion_cove_figures/02_lunar_lake_ring_bus.png)
...
![图 3：Lion Cove、Redwood Cove 与 Zen 5 的 Cache/内存延迟](intel_lion_cove_figures/03_l3_latency.png)
...
![图 4：带宽压力下的 DRAM 延迟](intel_lion_cove_figures/04_loaded_dram_latency.png)

*图 4：横轴逐步增加并发内存负载，纵轴观察随机访问延迟。Lunar Lake 在一段中等负载下保持得不错，但这不是纯核心属性：封装内 LPDDR5X、内存控制器、NoC、预取器和功耗管理都会参与结果。*

![图 5：Lunar Lake 的封装与内存布局](intel_lion_cove_figures/05_lunar_lake_package.jpg)
...
![图 6：8 MB Memory Side Cache 的访问延迟](intel_lion_cove_figures/06_memory_side_cache_latency.png)
...
![图 7：多核 DRAM 读取带宽](intel_lion_cove_figures/07_multicore_dram_bandwidth.png)
...
![图 8：历代 Intel P-Core 的 L2/L3 容量与延迟](intel_lion_cove_figures/08_l2_capacity_latency.jpg)
...
![图 9：Lion Cove 的多级数据 Cache 延迟](intel_lion_cove_figures/09_l1_l15_l2_latency.png)
...
![图 10：Lion Cove 的四级数据层次](intel_lion_cove_figures/10_cache_hierarchy.jpg)
...
![图 11：Lion Cove 私有 Cache 的带宽](intel_lion_cove_figures/11_private_cache_bandwidth.png)
...
![图 12：L3 与 DRAM 区间的带宽](intel_lion_cove_figures/12_l3_bandwidth.png)
...
![图 13：Lion Cove 调度器重组](intel_lion_cove_figures/13_scheduler_reorganization.png)
...
![图 14：调度器容量比较](intel_lion_cove_figures/14_scheduler_capacity.jpg)
...
![图 15：Lion Cove 的 18 个执行端口](intel_lion_cove_figures/15_execution_ports.png)
...
![图 16：ROB、寄存器与 Load/Store 队列容量](intel_lion_cove_figures/16_backend_resource_capacity.jpg)
...
![图 17：Lion Cove 的 Store-to-Load Forwarding 矩阵](intel_lion_cove_figures/17_store_forwarding_matrix.png)
...
![图 18：不同 Store Forwarding 情形的延迟](intel_lion_cove_figures/18_store_forwarding_latency.jpg)
...
![图 19：Lion Cove、Redwood Cove 与 Zen 5 的 TLB 组织](intel_lion_cove_figures/19_tlb_capacity.jpg)
...
![图 20：重命名阶段可消除或加速的操作](intel_lion_cove_figures/20_rename_optimization_throughput.jpg)
...
![图 21：Lion Cove 的取指、译码与微操作 Cache](intel_lion_cove_figures/21_frontend_fetch_decode.png)
...
![图 22：不同代码规模下的指令供给](intel_lion_cove_figures/22_instruction_fetch_bandwidth.png)
...
![图 23：Lion Cove 的随机方向模式预测](intel_lion_cove_figures/23_lion_cove_direction_pattern.png)

*图 23：横轴增加重复随机模式长度。单分支可维持到约 12K 的规模，随后成功率下降；这反映历史与预测表共同形成的有效能力，不能直接读成 12K 位全局历史。*

![图 24：Redwood Cove 的对应方向模式](intel_lion_cove_figures/24_redwood_cove_direction_pattern.png)

*图 24：Redwood Cove 的整体形态与 Lion Cove 接近，说明这一代重点不在大幅扩展方向历史。两张图的细微差异还可能来自散列、表冲突和训练状态。*

![图 25：SPEC CPU2017 整数子项的分支预测准确率](intel_lion_cove_figures/25_spec_integer_branch_accuracy.png)

*图 25：多数项目已经非常接近 100%，因此看似微小的准确率差异可能对应可观的 MPKI 相对变化。Zen 5 在 541.leela、557.xz 等困难子项更有优势，Lion Cove 与 Redwood Cove 的几何平均变化则很小。*

![图 26：SPEC CPU2017 浮点子项的分支预测准确率](intel_lion_cove_figures/26_spec_fp_branch_accuracy.png)
...
![图 27：连续 Taken 分支的吞吐](intel_lion_cove_figures/27_taken_branch_throughput.png)
...
![图 28：三级 BTB 的容量与命中延迟](intel_lion_cove_figures/28_btb_capacity_latency.png)
...
![图 29：返回地址栈深度与调用吞吐](intel_lion_cove_figures/29_return_stack_depth.png)
...
![图 30：Lion Cove 微架构总览](intel_lion_cove_figures/30_lion_cove_microarchitecture.png)

*图 30：核心入口和后端明显扩宽，Cache 层级也更细。框图中的容量与端口大多来自公开资料和微基准复原，不是 RTL 连接图；最值得关注的是资源之间能否形成端到端平衡。*

![图 31：Redwood Cove 微架构总览](intel_lion_cove_figures/31_redwood_cove_microarchitecture.png)
...
![图 32：SPEC CPU2017 整数与浮点总分](intel_lion_cove_figures/32_spec_cpu2017_total.png)
...
![图 33：Intel 对 Lion Cove 性能提升的公开表述](intel_lion_cove_figures/33_intel_lion_cove_performance_slide.jpg)
...
![图 34：SPEC CPU2017 整数子项](intel_lion_cove_figures/34_spec_integer_subscores.png)

*图 34：Lion Cove 相对 Redwood Cove 的提升分布不均，说明 23.2% 不是每个程序共享的固定 IPC 增幅。分支密度、指令工作集、Load 延迟和整数端口压力会改变各子项受益程度。*

![图 35：SPEC CPU2017 浮点子项](intel_lion_cove_figures/35_spec_fp_subscores.png)
...
![图 36：Lunar Lake 同时推进核心、工艺与系统结构变化](intel_lion_cove_figures/36_lunar_lake_innovation_slide.jpg)
