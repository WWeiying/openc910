# Cortex-A725 深度拆解：五宽乱序、精简前端与密度优化
...
![图 1：两台 Dell Pro Max with GB10](arm_cortex_a725_figures/01_dell_pro_max_gb10.jpg)
...
![图 2：Cortex-A725 微架构总览](arm_cortex_a725_figures/02_cortex_a725_overview.png)
...
![图 3：A725 的实现选项与 GB10 配置](arm_cortex_a725_figures/03_cortex_a725_configuration_options.jpg)
...
![图 4：A725 的随机分支模式识别曲面](arm_cortex_a725_figures/04_branch_random_pattern_a725.png)

*图 4：横轴为模式长度 2～32768，另一轴为循环中的分支数 1～512，纵轴为随机与可预测模式的纳秒差。曲面抬升说明这一组合已不能维持相同预测效果；它同时受历史长度、预测表容量、索引和混叠影响，不能据此认定某种具体算法。*

![图 5：A710 的相同分支模式测试](arm_cortex_a725_figures/05_branch_random_pattern_a710.png)
...
![图 6：A725 的 BTB 容量与延迟台阶](arm_cortex_a725_figures/06_btb_capacity_latency_comparison.png)
...
![图 7：A725、A710 与 Skymont 的目标供给对照](arm_cortex_a725_figures/07_taken_branch_throughput_a725_skymont.png)
...
![图 8：SPEC CPU2017 整数套件的分支正确率](arm_cortex_a725_figures/08_spec2017_integer_branch_prediction.png)
...
![图 9：SPEC CPU2017 浮点套件的分支正确率](arm_cortex_a725_figures/09_spec2017_fp_branch_prediction.png)
...
![图 10：A725、A710 与 Skymont 的指令供给带宽](arm_cortex_a725_figures/10_frontend_instruction_bandwidth.png)
...
![图 11：MOV 消除与清零惯用法吞吐](arm_cortex_a725_figures/11_move_elimination_test.jpg)
...
![图 12：A725、A710/N2 与 Skymont 的乱序资源](arm_cortex_a725_figures/12_out_of_order_structure_sizes.jpg)
...
![图 13：64-bit Store 到 32-bit Load 的对齐矩阵](arm_cortex_a725_figures/13_store_to_load_forwarding_alignment.png)
...
![图 14：顺序遍历数组时的数据侧 L2 TLB refill](arm_cortex_a725_figures/14_l2_dtlb_refill.png)
...
![图 15：A725 控制寄存器中的 MMU coalescing 模式](arm_cortex_a725_figures/15_mmu_coalescing_register.jpg)
...
![图 16：GB10 16 MB L3 簇上的 A725 单线程带宽](arm_cortex_a725_figures/16_l1d_bandwidth.png)
...
![图 17：Arm 优化指南给出的 A725 Cache 延迟模型](arm_cortex_a725_figures/17_cache_latency_optimization_guide.png)
...
![图 18：SPEC CPU2017 单副本估算总分](arm_cortex_a725_figures/18_spec2017_overall_performance.png)
...
![图 19：SPEC CPU2017 整数套件分项](arm_cortex_a725_figures/19_spec2017_integer_performance.png)
...
![图 20：SPEC CPU2017 浮点套件分项](arm_cortex_a725_figures/20_spec2017_fp_performance.png)
...
![图 21：高 IPC、核心受限的 SPEC 子项](arm_cortex_a725_figures/21_spec2017_core_bound.png)
...
![图 22：内存受限的 SPEC 子项](arm_cortex_a725_figures/22_spec2017_memory_bound.png)
...
核心受限程序中，性能可近似理解为 IPC × 频率；内存受限程序还要加入有效 Cache 容量、并发未命中数、互连和 DRAM 延迟。频率提高会增加每秒可执行周期，却也把固定纳秒内存延迟换算成更多周期。A725 在图 21 中展现架构效率；图 22 中，较低频率会把固定纳秒延迟折算成较少周期，GB10 的内存层次也会影响 IPC，但这些作用无法仅靠一张总分图拆开。
