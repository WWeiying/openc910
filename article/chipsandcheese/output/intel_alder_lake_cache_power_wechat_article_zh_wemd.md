...
themeName: "学术论文 (副本)"
title: "intel_alder_lake_cache_power_wechat_article_zh"
---
...
![图 1：Golden Cove 数据侧不同层级的单位传输能耗](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/390175f1d22a7d56_01_figure.png)

图 1 显示，从 DRAM 取数据的封装内能耗接近 L3 的 5 倍，而且尚未计入 DIMM 自身功耗。测试因 DDR5 主板问题使用 DDR4-3200，这一配置边界不能忽略。

![图 2：Golden Cove 指令侧取指能耗随工作集变化](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/705e42f02427759a_02_figure.png)
...
![图 3：Gracemont 与 Golden Cove 的数据侧能耗对比](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/cbadd193506f7ca5_03_figure.png)

Gracemont 虽支持 AVX，但 256-bit 操作会拆成两个 128-bit 操作，更小的数据粒度会延长搬运同等数据量所需时间。共享 L2 还要在四核请求间仲裁，面积利用率较好，却可能牺牲带宽和能效。

![图 4：Alder Lake 官方裸片图上的 Gracemont 到 L2/L3 推测路径](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/3f925f79029aacfa_04_figure.jpg)
...
![图 5：Gracemont 与 Golden Cove 的指令侧能耗](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/9c285b12b1d6540a_05_figure.png)
...
![图 6：Golden Cove 与 Skylake 的数据侧能耗和带宽；Skylake 使用双通道 DDR4-2133](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/2057a125754f6b2c_06_figure.png)
...
![图 7：Golden Cove 与 Haswell 数据侧对比；Haswell 使用双通道 DDR3-1333](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/2894995bd9b88a56_07_figure.png)

Haswell 虽仍有不错带宽，但 22 nm 工艺在各层级都更耗能，DDR3 平台也明显落后于更新的 DDR4 配置。跨代结果同时包含工艺、频率、缓存和内存差异，不能当作纯微架构对比。

![图 8：Golden Cove 与 Skylake 的指令侧能耗](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/328542e828f7fb59_08_figure.png)

指令侧 Golden Cove 总体领先；Skylake 的 L2 取指路径却异常高效。两代在 uop cache miss 后转走 L1I+译码时，能耗增加都很小。

![图 9：Golden Cove 与 Haswell 的指令侧能耗](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/af07d85029a993b5_09_figure.png)

Haswell 受旧工艺影响全面落后，而且测试没有观察到明显的 uop cache 能效收益，或收益落在误差范围内。

## 与 AMD 比较：Intel 私有缓存强，AMD 共享 L3 强

![图 10：Golden Cove 与 Zen 2 数据侧对比；Ryzen 9 3950X 使用 DDR4-3333](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/f1fc3010d2d8a389_10_figure.png)
...
![图 11：Golden Cove 与 Zen 2 指令侧能耗](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/f7c73fd81a95bc43_11_figure.png)

Golden Cove 从 uop cache 供给时略优；Zen 2 在 uop cache miss、转入 L1I+译码后能耗增加较明显。但 L1I miss 后由更低层缓存供给时，Zen 2 更高效；进入 DRAM 后，这一优势又消失。

![图 12：Golden Cove 与 Zen 3 数据侧对比；Ryzen 9 5950X 使用 DDR4-3600](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/c9b2df78e9081096_12_figure.png)

Zen 3 的私有缓存能效相对 Zen 2 回退，但 L3 仍以更高带宽和略低能耗胜出。可能因素包括更高默认 boost、更快内存带来的 Infinity Fabric 频率上升，以及 I/O die 功耗增加。

![图 13：Golden Cove 与 Zen 3 指令侧能耗](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/abe84b34ac5a9584_13_figure.png)
...
![图 14：Gracemont 与 Tremont 的数据侧能耗和带宽](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/a7b676288668684f_14_figure.png)
...
![图 15：Gracemont 与 Goldmont Plus 数据侧对比；J4125 为单通道 DDR4-2133](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/219af21e6f0d384c_15_figure.png)
...
![图 16：Gracemont 与 Tremont 的指令侧能耗](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/c9aeaf71c5bdd9e3_16_figure.png)

Tremont 在指令侧仍然最省能。

![图 17：Gracemont 与 Goldmont Plus 的指令侧能耗](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/74ba831ae3440c20_17_figure.png)
...
![图 18：Haswell 指令译码功耗研究中的缓存功耗占比](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/4e444330ac72f49a_18_figure.png)

图 18 来自一篇研究 Haswell 译码器功耗的论文，用来说明缓存本身就是显著功耗来源，并非本文微基准的新测量。

![图 19：各平台按层级汇总的单位能耗估算](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/13edc98cb4551a8b_19_figure.png)

图 19 按缓存层级着色，使用落在某级容量内、又超出上一级容量的数据点取平均；DRAM 使用 1—3 GB 工作集。经验上，L2 hit 约为 L1 的两倍，L3 往往又超过 L2 的两倍，DRAM 通常是 L3 的 4—5 倍。

![图 20：Arm CI-700 页面同样强调减少数据移动](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/1b0fc6b0c0480fd4_20_figure.jpg)
...
![图 21：大核在重 L3 流量下的封装功率变化](https://gongzhonghao1-1402552401.cos.ap-shanghai.myqcloud.com/wechat/articles/intel_alder_lake_cache_power_wechat_article_zh/7be2f1dfb9fbf156_21_figure.png)
