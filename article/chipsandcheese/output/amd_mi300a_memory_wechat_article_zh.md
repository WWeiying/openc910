...
![图 1：MI300 的模块化构造，计算 die 可在 CCD 与 XCD 之间组合](amd_mi300a_memory_figures/01_figure.jpg)
...
![图 2：四路测试系统的 lstopo 拓扑，每颗 MI300A 作为一个 NUMA node](amd_mi300a_memory_figures/02_figure.jpg)
...
![图 5：单路 MI300A 的依赖读取延迟阶梯，Infinity Cache 命中超过 140 ns，HBM3 约 227 ns](amd_mi300a_memory_figures/05_figure.jpg)
...
![图 6：MI300A 中请求在 CM、CS、2 MB slice 与 HBM 间的路径示意](amd_mi300a_memory_figures/06_figure.png)
...
![图 7：四个 NUMA node 的 2 GB 读取延迟矩阵，本地约 234—247 ns，远端约 480—580 ns](amd_mi300a_memory_figures/07_figure.png)

`numactl --hardware` 报告各 node 距离相同，GIGABYTE 图也暗示四颗芯片彼此直连；实测却有成对差异。

![图 8：G383-R80-AAP1 方框图，四颗 MI300A 之间有封装外 Infinity Fabric 链路](amd_mi300a_memory_figures/08_figure.jpg)
...
![图 9：本地、近端和远端 node 的缓存/内存延迟曲线](amd_mi300a_memory_figures/09_figure.jpg)
...
![图 10：访问远端 node Infinity Cache 时，请求必须跨封装到拥有该地址的 CS](amd_mi300a_memory_figures/10_figure.png)
...
![图 11：远端 Infinity Cache miss 后继续访问归属 node HBM3 的完整路径](amd_mi300a_memory_figures/11_figure.png)

与常规双路服务器相比，MI300A 的本地和远端 DRAM 延迟都很高，Zen 4 CPU 核因此会落后于桌面或 EPYC 实现。

![图 12：MI300A、双路 EPYC 9Y3X 与双路 Xeon 的本地/远端 DRAM 延迟对比](amd_mi300a_memory_figures/12_figure.jpg)

## 一致性 probe 与核间传递

Infinity Cache 和 HBM 是主要数据来源。若 probe filter 显示某地址已在其他 CPU/GPU die 中修改，CS 还要发 probe，确保所有计算单元看到一致数据。这类流量占比很小，却决定共享内存语义。

![图 13：四路 MI300A 的 core-to-core latency 全矩阵缩略图](amd_mi300a_memory_figures/13_figure.jpg)

单颗 MI300A 内跨 CCD 传递约 260—450 ns，差异可能取决于测试地址 homed 到哪个 CS。它明显慢于 EPYC，但 MI300A 的 Fabric 网络也大得多。

![图 14：MI300A 与桌面/服务器 Zen 4 的 CM、CS 数量及核间传递规模对比](amd_mi300a_memory_figures/14_figure.jpg)

四颗芯片组成的一致性域更加复杂。最坏情况是负责地址的 CS 与两个通信 core 都不在同一 node：传递会涉及三颗 MI300A，一颗持有请求方，一颗持有数据方，第三颗负责 home/orchestration。

![图 15：四路系统逐核延迟热图，跨 node 特别是第三方 home node 路径最慢](amd_mi300a_memory_figures/15_figure.png)

这也解释 AMD 为什么更愿意在单 socket 内提高 EPYC 核数，而不是继续扩到四路。只看单颗 MI300A，拓扑会清晰很多。

![图 16：单个 MI300A node 内的逐核延迟，每次只测一个核心](amd_mi300a_memory_figures/16_figure.jpg)

![图 17：AWS 上 12 核 Zen 4 实例的核间延迟对照](amd_mi300a_memory_figures/17_figure.jpg)
...
![图 18：MI300A CCD、桌面 Ryzen 7950X 与 EPYC CCD 到 I/O die 的链路宽度对比](amd_mi300a_memory_figures/18_figure.jpg)
...
![图 19：同 CCD 与其他 CCD 加载内存时的延迟；同 CCD 流量超过约 71 GB/s 后延迟激增](amd_mi300a_memory_figures/19_figure.png)

即便另外两块 CCD 合计读取超过 140 GB/s，第一块 CCD 的延迟也不变。三块 CCD 无法接近 HBM3 极限。

![图 20：24 个 CPU 核和 6 个 GPU XCD 在 MI300A 内的连接关系](amd_mi300a_memory_figures/20_figure.jpg)
...
![图 21：Y-Cruncher 总计算时间，MI300A 慢于 EPYC 9R14，快于 7950X](amd_mi300a_memory_figures/21_figure.png)

![图 22：SPEC CPU2017 估算分数；MI300A 单线程整数大致处于 Ryzen 9 3950X 的 Zen 2 水平](amd_mi300a_memory_figures/22_figure.png)
...
![图 23：跨 node 读取带宽随工作集变化；命中远端 Infinity Cache 时接近链路能力](amd_mi300a_memory_figures/23_figure.png)

工作集扩大到远端 HBM 后，CPU 读取带宽却很低。作者尝试让所有线程按 flag 同时结束，避免固定工作量带来的尾部误差，也尝试 2 MB huge page 减少地址翻译开销。

![图 24：读取远端 node Infinity Cache 的路径与约 112—115 GB/s 结果](amd_mi300a_memory_figures/24_figure.png)

这些改动都无明显作用，remote DRAM read 仍只有 25—26 GB/s。

![图 25：Infinity Cache miss 后读取远端 HBM3 的路径，CPU 实测约 25 GB/s](amd_mi300a_memory_figures/25_figure.png)
...
![图 26：跨 node non-temporal write 绕过 Infinity Cache，实测约 45.43 GB/s](amd_mi300a_memory_figures/26_figure.png)

45.43 GB/s 好于 25 GB/s，却仍远低于链路已证明可达的 112 GB/s。改变线程数也没有解决。

![图 27：remote bandwidth 随线程数扩展；超过两个 core 后不再上升](amd_mi300a_memory_figures/27_figure.png)
...
![图 28：OpenCL buffer copy 带宽，本地与跨 MI300A GPU 的大块复制可达约 55.9 GB/s](amd_mi300a_memory_figures/28_figure.png)
...
![图 29：AMD 对 MI300A coherent shared memory 的说明](amd_mi300a_memory_figures/29_figure.png)

驱动仍可能在 SVM 背后复制整块 buffer。为识别 zero copy，测试分配 256 MB，却只改一个 32-bit value：真正共享只需传播一个 cache line；若驱动复制整块，延迟会进入毫秒级。

![图 30：256 MB SVM buffer 只修改 32 bit 时的 CPU↔GPU 同步延迟](amd_mi300a_memory_figures/30_figure.png)
...
![图 31：改用 4 KB allocation 后的同步延迟，显示微秒级主要来自同步固定开销](amd_mi300a_memory_figures/31_figure.png)
...
![图 34：同一 Zen 4 CCD 在 EPYC、MI300A 中复用，以及 IOD/XCD 的模块化接口](amd_mi300a_memory_figures/34_figure.jpg)

多种先进封装共同支撑它：die 堆在 die 上，再堆到另一层 die；垂直堆叠由 TSV 提供高带宽，相邻 IOD 也用超短距离链路连接。

![图 35：MI300 的 3D hybrid bonding、2.5D interposer 与多层封装](amd_mi300a_memory_figures/35_figure.jpg)
...
![图 37：MI300A 的 24 个 Zen 4 核、三个 CCD 与共享 HBM/Infinity Cache](amd_mi300a_memory_figures/37_figure.jpg)

更重要的是，CPU 可以快速与 GPU 共享数据。若长时间 GPU kernel 之间需要少量 CPU 处理，24 核可能正好喂饱接近 MI300X 规模的 GPU，无需独立 host CPU。

![图 38：El Capitan 使用四路 MI300A node，并在 2024 年 11 月 TOP500 位居第一](amd_mi300a_memory_figures/38_figure.jpg)
