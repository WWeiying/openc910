...
![图 1：Zen 5 执行两条 512-bit FMA 时的性能轨迹](amd_zen5_avx512_figures/01_figure.jpg)

*图 1：两条寄存器操作数 FP32 FMA 已能占满两条 512-bit FMA 管线，但在被测快速 CCD 上几乎没有触发持续降速。*

![图 2：寄存器 FMA 测试的局部放大](amd_zen5_avx512_figures/02_figure.jpg)
...
![图 3：Skylake-X 的标量与 AVX-512 频率变化](amd_zen5_avx512_figures/03_figure.jpg)

*图 3：Skylake-X 呈现比较清晰的标量档位、切换阶段和 AVX-512 稳态档位。*

![图 4：Skylake-X 转换窗口放大](amd_zen5_avx512_figures/04_figure.jpg)
...
![图 5：快速 CCD 上的双 FMA 内存操作数测试](amd_zen5_avx512_figures/05_figure.jpg)

*图 5：表面吞吐一度只相当于约 4.7 GHz，且持续时间远长于 Skylake-X 的微秒级切换。*

![图 6：较慢 CCD 没有出现同样的转换](amd_zen5_avx512_figures/06_figure.jpg)

*图 6：9900X 的 CCD0 最高约 5.7 GHz，CCD1 最高约 5.4 GHz；后者留出的电气余量更大，没有重现相同慢段。*

![图 7：不同核心的转换差异](amd_zen5_avx512_figures/07_figure.jpg)
...
![图 8：硬件频率计数与 IPC 的分离](amd_zen5_avx512_figures/08_figure.jpg)

*图 8：频率仍接近峰值时，IPC 已显著低于理论值；软件看到的工作完成速度不能简单等同于时钟。*

![图 9：短采样中的测量误差](amd_zen5_avx512_figures/09_figure.png)

*图 9：`perf` 调用开销会给很短的迭代带来约 1%～3% 误差，因此单个尖点不宜过度解释。*

![图 10：节流行为的核心间差异](amd_zen5_avx512_figures/10_figure.jpg)
...
![图 11：加入第三条向量运算后的轨迹](amd_zen5_avx512_figures/11_figure.jpg)

![图 12：更重负载下的第二组记录](amd_zen5_avx512_figures/12_figure.jpg)

*图 11、12：更高开关活动延长了控制环路的限制阶段，也压低了最终稳态频率。*

![图 13：三管线压力测试的局部放大](amd_zen5_avx512_figures/13_figure.jpg)
...
![图 14：快速交替时没有反复触发完整惩罚](amd_zen5_avx512_figures/14_figure.jpg)

![图 15：频率恢复过程](amd_zen5_avx512_figures/15_figure.jpg)

![图 16：延长标量间隔后的软转换](amd_zen5_avx512_figures/16_figure.jpg)

![图 17：更长间隔重新触发完整阶段](amd_zen5_avx512_figures/17_figure.jpg)

![图 18：完全回到峰值需要超过 100 ms](amd_zen5_avx512_figures/18_figure.jpg)

*图 14～18：恢复路径带有明显迟滞。它牺牲短时间峰值，换取负载反复切换时不必持续经历严重节流。*

![图 19：转换轨迹与电压下陷控制的相似性](amd_zen5_avx512_figures/19_figure.jpg)
...
![图 20：FP 非调度队列满周期](amd_zen5_avx512_figures/20_figure.jpg)

*图 20：队列满与 IPC 低谷同时出现，是“后端入口受限”的旁证；它不能单独区分调度选择、供电门控或执行端口限制。*

![图 21：被测 Ryzen 处理器](amd_zen5_avx512_figures/21_figure.jpg)

![图 22：处理器背面触点](amd_zen5_avx512_figures/22_figure.jpg)
