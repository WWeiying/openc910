...
![图 1：NetBurst 与 Pentium III、Athlon 64、Merom 的核心结构规模](intel_netburst_figures/01_figure.png)

NetBurst 相比 P6 Pentium III 拥有更多执行资源和数倍大的乱序结构，也大于同期 Athlon 64，甚至大于后继 Merom。

![图 2：P6 与 NetBurst 三发射流水线的资源对比](intel_netburst_figures/02_figure.png)
...
![图 3：Prescott 裸片上的前端与预测相关区域；裸片图来自 Martijn Boer](intel_netburst_figures/03_figure.jpg)
...
![图 4：不同历史模式长度下的预测能力](intel_netburst_figures/04_figure.png)

![图 5：NetBurst 与后继核心的相关模式预测对比](intel_netburst_figures/05_figure.png)
...
![图 6：小分支 footprint 下的 taken branch 吞吐](intel_netburst_figures/06_figure.png)

一旦超过快速跟踪能力，处理器可能转到 4096-entry 主 BTB，每条 taken branch 超过 13 周期，即约 12 个 bubble。

![图 7：越过快速层后的主 BTB 路径](intel_netburst_figures/07_figure.png)

再发生 BTB miss，代价高达 36 周期；此时可能从 L2 取指并用 branch address calculator 求目标。

![图 8：多代处理器 taken branch 代价；纵轴截在 20 周期以免 NetBurst 破坏比例](intel_netburst_figures/08_figure.png)
...
![图 10：8-byte NOP 的前端带宽；Goldmont Plus 与 Haswell L1I 均为 16 B/cycle](intel_netburst_figures/10_figure.png)
...
![图 11：大指令 footprint 下 NetBurst 与传统 32 KB L1I/3-wide Goldmont Plus 的吞吐](intel_netburst_figures/11_figure.png)
...
![图 12：trace cache、microcode ROM 与推测 renamer 位置](intel_netburst_figures/12_figure.jpg)

NetBurst rename/allocate 为 3-wide。它似乎不做 move elimination，却能识别 zeroing idiom、打断依赖，而且不同 zero idiom 表现不同。

![图 13：寄存器 MOV 与多种归零写法的吞吐](intel_netburst_figures/13_figure.png)
...
![图 14：裸片上清晰可见的独立寄存器文件](intel_netburst_figures/14_figure.jpg)
...
![图 15：NetBurst 与多代核心的调度容量](intel_netburst_figures/15_figure.png)

![图 16：分布式 math scheduler 与共享 dispatch port](intel_netburst_figures/16_figure.png)
...
![图 17：按 Henry Wong 方法测试 Pentium EE 965 的 store-to-load forwarding](intel_netburst_figures/17_figure.png)

若 load 被 store 覆盖但起始地址不同，超过 50 周期；load 跨 line 为 89 周期，store 跨 line 125 周期，两者都跨 line 高达 165 周期。它们虽是边界情况，其他核心通常只需数十周期。

![图 18：Goldmont Plus 同类测试；4-byte 粒度模式相近，但惩罚小得多](intel_netburst_figures/18_figure.png)
...
![图 19：NetBurst、Athlon 与后继核心的数据缓存参数](intel_netburst_figures/19_figure.jpg)

![图 20：Pentium EE 965 与 Athlon X2 6000+ 的纳秒延迟](intel_netburst_figures/20_figure.png)
...
![图 21：L1/L2 延迟与容量对比](intel_netburst_figures/21_figure.png)

当时多为两级缓存，L2 miss 通过 front-side bus 到北桥内存控制器，额外片外 hop 让 Pentium EE 与 Core 2 都比集成内存控制器的 Athlon 延迟高。

![图 22：4 KB 与 2 MB 页下的延迟差](intel_netburst_figures/22_figure.png)
...
![图 23：NetBurst 与 Pentium M 单线程缓存带宽](intel_netburst_figures/23_figure.png)

对 Westmere 则各级落后；后者 L3 容量大数倍，单线程带宽仍接近 NetBurst L2。NetBurst 唯一亮点是 16 B/cycle 配合高频，使 L1D 绝对带宽略高。

![图 24：NetBurst 与 Westmere 单线程带宽](intel_netburst_figures/24_figure.png)

![图 25：双核四线程 Pentium EE 965 与四核 Goldmont Plus 聚合带宽](intel_netburst_figures/25_figure.png)
...
![图 26：NetBurst 读写带宽；写带宽不足读的四分之一](intel_netburst_figures/26_figure.png)
...
![图 27：采用 write-back L1D 的常规核心读写带宽对照](intel_netburst_figures/27_figure.png)
...
![图 29：NetBurst 相对 P6 几乎所有主要模块都重做](intel_netburst_figures/29_figure.jpg)
...
![图 30：NetBurst 技术如何在 Sandy Bridge 重新组合](intel_netburst_figures/30_figure.jpg)
...
![图 31：Northwood 与 Cedar Mill 按周期的缓存延迟](intel_netburst_figures/31_figure.png)

![图 32：两者按纳秒的缓存延迟](intel_netburst_figures/32_figure.png)
