...
![图 1：Cannon Lake/Palm Cove 与 Kaby Lake/Skylake 等名称对应](intel_cannon_lake_figures/01_figure.jpg)
...
![图 2：Intel 2017 年新闻资料中的制程密度主张](intel_cannon_lake_figures/02_figure.jpg)

libx264 是 Palm Cove 的有利场景，因为能使用 AVX-512。测试锁定 2 核 4 线程，并分别比较封装功率与核心功率。

![图 3：libx264 在 Cannon Lake、Kaby Lake 与 Goldmont Plus 上的性能—封装功率](intel_cannon_lake_figures/03_figure.png)
...
![图 4：同一负载按核心功率比较](intel_cannon_lake_figures/04_figure.png)
...
![图 5：7-Zip 性能—封装功率](intel_cannon_lake_figures/05_figure.png)

Cannon Lake 在 7 W 以上略胜 Gemini Lake，但远非新节点应有的压倒性优势。

![图 6：7-Zip 性能—核心功率](intel_cannon_lake_figures/06_figure.png)

去掉 AVX-512 后，Palm Cove 连核心功率也落后 Kaby Lake，且后者能扩展到更高性能。libx264 的部分优势因此更像指令集收益。

![图 7：Fritzchens Fritz 裸片图中按同尺度裁出的 Palm Cove 与 Kaby Lake 核心](intel_cannon_lake_figures/07_figure.png)
...
![图 8：Cannon Lake 裸片功能区推测标注](intel_cannon_lake_figures/08_figure.jpg)
...
![图 9：Palm Cove 核心结构；除实测差异外按 Skylake 假设](intel_cannon_lake_figures/09_figure.png)

这些变化很小，性能影响可能难以测量，但比以往纯工艺 Tick 完全不改结构更积极。图中未实测部分不能当作 Palm Cove 官方参数。

## 分支预测：BTB 略大，密集分支稍有改善

![图 10：不同分支数量与间距下的 taken branch 吞吐](intel_cannon_lake_figures/10_figure.png)
...
![图 11：小于 32 B 间距时 Palm Cove 与 Skylake 的 taken branch 代价](intel_cannon_lake_figures/11_figure.png)
...
![图 12：Palm Cove 向量指令延迟与吞吐；512-bit FMA 约 4.85 个实测周期](intel_cannon_lake_figures/12_figure.jpg)

uops.info 报告 FMA 4 周期，本测试接近 5 周期。另一种可能是测试中核心从 3.2 降到 2.65 GHz，软件反推把降频算进延迟；缺少频率轨迹，无法判定。

![图 13：混合 256-bit 与 512-bit FMA 的吞吐](intel_cannon_lake_figures/13_figure.png)

Palm Cove 会把已有两条 256-bit FMA 数据通路融合成一条 512-bit 操作，理论上混合宽度时有机会超过 1 IPC，类似 Centaur CNS 和 Zen 4；实测却只有 1 IPC。

![图 14：两条 256-bit FMA 与一条融合 512-bit FMA 的模式示意](intel_cannon_lake_figures/14_figure.png)
...
![图 15：推测的 256-bit 向量执行数据流](intel_cannon_lake_figures/15_figure.jpg)

![图 16：推测的 512-bit FMA 融合数据流](intel_cannon_lake_figures/16_figure.jpg)

图 15—16 是把微基准映射到裸片布局的解释；512-bit 与混合宽度时都可能保持融合模式。

![图 17：Palm Cove（上）与 Coffee Lake/Skylake（下）的向量执行区](intel_cannon_lake_figures/17_figure.jpg)

![图 18：两代核心单个 64-bit lane 的对比](intel_cannon_lake_figures/18_figure.jpg)
...
![图 19：Palm Cove 相对 Skylake 的主要裸片差异，L2 区域用途未知](intel_cannon_lake_figures/19_figure.jpg)

Load/store 与整数数据通路之间某结构也变宽，可能是 pending store data buffer；下面用带宽检验，不据此直接命名。

## L1/L2 与内存带宽

![图 20：单核 L1D 读带宽](intel_cannon_lake_figures/20_figure.png)
...
![图 21：单核缓存层级带宽](intel_cannon_lake_figures/21_figure.png)

L2 持续带宽略超 32 B/cycle。客户端 Skylake 虽也有 64 B/cycle L2 datapath，AVX 可见带宽受 L1D 与 L2 prefetcher 干扰限制。

![图 22：全线程内存带宽；7700K 为 DDR4-2400，8121U 为 LPDDR4-2400，均 128-bit 接口](intel_cannon_lake_figures/22_figure.png)

四核 Kaby Lake 高频运行，但两核 Cannon Lake 也足以接近内存上限。4 GB 工作集下分别为 30.19 与 30.73 GB/s；相对理论值约 78.6% 与 80%，差异很小，两套控制器带宽效率都不错。

## 缓存与 DRAM 延迟

![图 23：按核心周期计的缓存/内存延迟](intel_cannon_lake_figures/23_figure.png)
...
![图 24：换算为纳秒的缓存/内存延迟](intel_cannon_lake_figures/24_figure.png)
...
![图 25：Cannon Lake Gen 10 iGPU 区域](intel_cannon_lake_figures/25_figure.jpg)

可见 5 个 subslice，每个 8 个 EU；Skylake/Kaby Lake GT2 同样每 subslice 8 EU，却只有 3 个 subslice。

![图 26：Kaby Lake GT2 对照](intel_cannon_lake_figures/26_figure.jpg)

![图 27：Intel 客户端 iGPU 规模演进](intel_cannon_lake_figures/27_figure.jpg)

Ice Lake 后来以四核产品带来更大 GT2，Cannon Lake 显示扩张从 Gen 10 已开始。

![图 28：Cannon Lake 与 Kaby Lake 的 iGPU L3 位置；红框为 L3，绿框为 subslice](intel_cannon_lake_figures/28_figure.jpg)
...
![图 29：Coffee Lake 与 Cannon Lake subslice 近旁 SRAM 对比，未按同尺度](intel_cannon_lake_figures/29_figure.jpg)
...
![图 30：Cannon Lake NUC 里与 CPU 封装并列的独立 GPU](intel_cannon_lake_figures/30_figure.jpg)
...
![图 31：Cannon Lake System Agent 与显示控制器区域](intel_cannon_lake_figures/31_figure.jpg)

Cannon Lake 显示控制器有更多逻辑和 SRAM。Intel 说明 SRAM 可缓存屏幕较大区域，以 burst 访问内存，让控制器在两次 burst 间进入低功耗。

![图 32：同尺度 Cannon Lake（左）与 Kaby Lake（右）显示控制器](intel_cannon_lake_figures/32_figure.jpg)

更宽 pixel pipe 也能用更低频工作，以面积换能效；Gen 11 白皮书确认过这种取舍。

![图 33：各代显示引擎能力演进](intel_cannon_lake_figures/33_figure.jpg)
...
![图 34：Intel Ice Lake 裸片中的 IPU，与 Cannon Lake 形态相近](intel_cannon_lake_figures/34_figure.jpg)
