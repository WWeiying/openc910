...
![图 1：Cuzco 的市场定位与总体特征](condor_cuzco_figures/01_figure.jpg)
...
![图 4：分支预测、BTB、取指与译码资源](condor_cuzco_figures/04_figure.jpg)
...
![图 5：重命名阶段生成执行时间表](condor_cuzco_figures/05_figure.jpg)

这与 Nvidia Kepler 以后 GPU 的静态调度有相似之处，但 Cuzco 由硬件动态建表，软件仍看到普通 RISC-V。后端不必每周期对所有候选项做复杂唤醒选择，只需等待预定周期再发射。

![图 6：时间资源矩阵 TRM](condor_cuzco_figures/06_figure.jpg)
...
![图 7：有限搜索窗口与理想贪心调度的性能差异](condor_cuzco_figures/07_figure.jpg)

Condor 的模拟显示，有限窗口相比假想的完美“贪心”时间表损失数个百分点。但完美调度本身未必可实现；传统分布式动态调度器也会因某个队列拥塞，而让其他队列背后的空闲单元用不上。

![图 8：变量延迟通过 poison 与 replay 处理](condor_cuzco_figures/08_figure.jpg)
...
![图 9：执行切片、XEQ 和 poison replay](condor_cuzco_figures/09_figure.jpg)
...
![图 10：Load/Store 队列与四条访存流水线](condor_cuzco_figures/10_figure.jpg)
...
![图 11：地址翻译和私有 L2 配置](condor_cuzco_figures/11_figure.jpg)
...
![图 12：八核集群、L3 分片和 CHI 出口](condor_cuzco_figures/12_figure.jpg)
