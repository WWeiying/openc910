...
![图 1：Graviton 3 与 V1 关系及系统概览](aws_graviton3_first_impressions_figures/01_figure.png)

## 分支预测：从 N1 的弱项变成强项

模式识别测试显示，Graviton 3 与 N1 已不是同一档。

![图 2：Graviton 3 的单 Branch Pattern 识别](aws_graviton3_first_impressions_figures/02_figure.png)

![图 3：Graviton 3 与 N1 的模式容量细节](aws_graviton3_first_impressions_figures/03_figure.jpg)

Zen 3 像是用稍慢、但极强的后级覆盖初级结果；Ice Lake 与 Graviton 3 更像单层长历史方案。这里是根据曲线作出的行为分类，不能据此确定算法。

![图 4：与 Ice Lake、Zen 3 的方向模式比较](aws_graviton3_first_impressions_figures/04_figure.png)

![图 5：不同历史长度下的误预测拐点](aws_graviton3_first_impressions_figures/05_figure.jpg)

同时存在 512 个 Branch 时，Graviton 3 能记长度约 16 的 Pattern，Ice Lake 约 32，Zen 3 约 96。

![图 6：多 Branch 条件下的 Pattern Capacity](aws_graviton3_first_impressions_figures/06_figure.png)

![图 7：512 Branch 时三种预测器的对照](aws_graviton3_first_impressions_figures/07_figure.jpg)

目标侧同样激进。Micro-BTB 可每拍处理两个 Taken Branch；当时只有 Golden Cove、Rocket Lake 也测出过该能力，而且它们的相应容量约 32 项和 8 项，Graviton 3 更大。

![图 8：Micro-BTB 双 Taken/cycle 的容量区间](aws_graviton3_first_impressions_figures/08_figure.png)
...
![图 9：不同 BTB Footprint 下的 Taken 吞吐](aws_graviton3_first_impressions_figures/09_figure.png)
...
![图 10：Decode、Micro-op Cache 与 Rename 前端](aws_graviton3_first_impressions_figures/10_figure.png)

Decoder 支持 Flag-setting Instruction+Conditional Jump 融合，也会把成对 NOP 融合。缓存融合结果后，NOP 测试可达荒诞的 12 IPC，破坏常规取指带宽测法。

![图 11：NOP Fusion 导致的 12 IPC](aws_graviton3_first_impressions_figures/11_figure.png)
...
![图 12：不同代码 Footprint 的取指吞吐](aws_graviton3_first_impressions_figures/12_figure.png)

## Rename 优化与物理资源

Rename 看来为六宽。它不能像 Zen 3/Sunny Cove 那样按 Rename Width 消除 Register-to-register MOV，吞吐仍受 ALU 端口限制；偶尔能断开 MOV 依赖，但能力有限。

![图 13：MOV 与 Zeroing Idiom 的吞吐](aws_graviton3_first_impressions_figures/13_figure.jpg)
...
![图 14：ROB 容量测试与 256/512 两种解释](aws_graviton3_first_impressions_figures/14_figure.png)

Vector PRF 看来有约 125 个 256 bit Register，可用一个 Entry 跟踪两个 Scalar FP，却不能同样容纳两个 128 bit NEON Result。

![图 15：Scalar FP、NEON、SVE 的 Register Capacity](aws_graviton3_first_impressions_figures/15_figure.png)

除巨大的 Scalar FP Rename Capacity 外，PRF 更像为 256 项 ROB 配置；Load Queue 则异常大，其他队列大致接近 Zen 2/3。

![图 16：ROB、PRF、Load/Store Queue 容量对比](aws_graviton3_first_impressions_figures/16_figure.png)

分布式 Scheduler 很难反推，作者只给几类指令的近似容量。

![图 17：Scheduler Capacity 的多组定向测试](aws_graviton3_first_impressions_figures/17_figure.png)

![图 18：常见操作可用的 Scheduler Entry](aws_graviton3_first_impressions_figures/18_figure.jpg)

整体明显大于 N1，并与 Zen 3/Ice Lake 常见操作资源相近。

![图 19：一种可能的 Scheduler 拓扑](aws_graviton3_first_impressions_figures/19_figure.png)
...
![图 20：常见 Scalar/Vector 操作延迟](aws_graviton3_first_impressions_figures/20_figure.jpg)
...
![图 21：NEON/SVE 执行吞吐与潜在 RF 限制](aws_graviton3_first_impressions_figures/21_figure.jpg)
...
![图 22：按周期计的 Cache/Memory Latency](aws_graviton3_first_impressions_figures/22_figure.png)

Graviton 2 与 Altra 同为 N1/L2，主要差别在 Mesh/L3；Graviton 2 的较小 Mesh 略好。

![图 23：折算为 ns 的 Graviton 2/3 与 Altra 延迟](aws_graviton3_first_impressions_figures/23_figure.png)
...
![图 24：三种 Server Cache Strategy](aws_graviton3_first_impressions_figures/24_figure.png)

Zen 3 与 Sunny Cove 还要服务 Client，可在 4 GHz 以上；Milan/Ice Lake 云实例的频率明显高于 Graviton 3。按纳秒看，低频让 Arm 的周期优势缩小。

![图 25：以实际时间比较 L1、L2、L3 与 DRAM](aws_graviton3_first_impressions_figures/25_figure.png)
...
![图 26：Graviton 3 与 Altra 的 Cache Bandwidth](aws_graviton3_first_impressions_figures/26_figure.png)

x86 对手依靠高频与同/更宽 Vector 仍更强；Ice Lake 的 512 bit 和 64 B/cycle L2 尤其突出。256 bit SVE 能缩小差距，不能消除频率差。

![图 27：Graviton 3、Zen 3、Ice Lake 的每核带宽](aws_graviton3_first_impressions_figures/27_figure.png)

DDR5 的高延迟换来高带宽，Graviton 3 在 Memory Bandwidth 明显领先。

![图 28：整芯片可获得数据下的层级带宽](aws_graviton3_first_impressions_figures/28_figure.png)
