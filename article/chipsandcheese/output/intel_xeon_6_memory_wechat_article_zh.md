...
![图 1：历代 Xeon Scalable 的 Die 组织](intel_xeon_6_memory_figures/01_figure.png)
...
![图 2：Xeon 6 Compute Die](intel_xeon_6_memory_figures/02_figure.jpg)
...
![图 3：Xeon 6 模块化 SoC 与 Mesh](intel_xeon_6_memory_figures/03_figure.jpg)
...
![图 4：Xeon 6 与 Meteor Lake 的 Cache 延迟](intel_xeon_6_memory_figures/04_figure.png)

*图 4：核心私有层次基本延续 Redwood Cove，服务器从 L3 起明显不同。160 MB 本地池容量巨大，代价是约 33 ns 的命中。*

![图 5：Xeon 6、Emerald Rapids 与 Sapphire Rapids](intel_xeon_6_memory_figures/05_figure.png)
...
![图 6：Xeon 6 与 AMD Zen 5 Server 的延迟对照](intel_xeon_6_memory_figures/06_figure.png)
...
![图 7：单核 Cache/DRAM 带宽](intel_xeon_6_memory_figures/07_figure.png)
...
![图 8：整片 Cache 与 DRAM 带宽](intel_xeon_6_memory_figures/08_figure.png)
...
![图 9：本地、相邻与最远 L3 命中](intel_xeon_6_memory_figures/09_figure.png)

*图 9：本地约 33.25 ns，跨一颗 Die 约 57.63 ns，跨两颗接近 80 ns。每条边界增加约 24～25 ns。*

![图 10：远端 DRAM 与受载延迟](intel_xeon_6_memory_figures/10_figure.png)
...
![图 11：跨 Die L3 带宽路径示意](intel_xeon_6_memory_figures/11_figure.png)
...
![图 12：Xeon 6 的核间延迟矩阵](intel_xeon_6_memory_figures/12_figure.jpg)
...
![图 13：Xeon 6 6985P-C 的 SPEC CPU2017 单线程估算](intel_xeon_6_memory_figures/13_figure.png)

*图 13：整数约 7.19、浮点约 10.54；Graviton 4 约 7.38/9.72。云平台、编译器和 ISA 不同，结果用于平台级观察，不是 Redwood Cove 与 Arm 核心的纯 IPC 对照。*

![图 14：Intel 对逻辑统一架构的说明](intel_xeon_6_memory_figures/14_figure.jpg)

*图 14：幻灯片原本展示 Emerald Rapids，但目标同样适用于 Xeon 6：Mesh 跨越 Die 边界，让所有核心访问共享 L3 与内存控制器。*

Xeon 6 的优势包括更大、更统一的 L3，更低的核间延迟，以及没有 CCD 边界上的窄链路。代价也很清楚：L3 本地就要约 33 ns，单核 L3 带宽甚至低于 Zen 5 单核 DRAM；控制器位于 Compute Die 的 DRAM 延迟优势也没有兑现。
...
Chester Lam 没有给出“Intel 路线必然不值得”的结论。相反，Xeon 6 的工程实现相当 impressive；真正的问题是，随着 Mesh 继续变大，维持一个单层逻辑单体所付出的延迟、功耗和验证成本，是否仍小于它带来的软件与共享 Cache 收益。
