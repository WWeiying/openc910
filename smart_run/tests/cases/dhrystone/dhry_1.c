...
#ifdef SMART_RUN
#include "vtimer.h"
#ifndef DHRYSTONE_RUNS
#define DHRYSTONE_RUNS 1000
#endif
#endif
  ...
  /* Main and Proc_0 in the Ada version             */
{
...
#ifdef SMART_RUN
  Next_Ptr_Glob = &Next_Rec_Glob_Store;
  Ptr_Glob = &Rec_Glob_Store;
#else
  ...
  Dhrystones_Per_Second = 1000000.0f * (float) Number_Of_Runs /
                          (float) User_Time;
  printf ("Cycles for one run through Dhrystone:       ");
  printf ("%6.1f \n", Microseconds);
  printf ("Dhrystones per simulated MHz:               ");
  printf ("%6.1f \n", Dhrystones_Per_Second);
  printf ("DMIPS/MHz:                                  ");
  printf ("%6.3f \n", Dhrystones_Per_Second / 1757.0f);
  printf ("\n");
  if (Int_Glob == 5 &&
      Bool_Glob == 1 &&
      Ch_1_Glob == 'A' &&
