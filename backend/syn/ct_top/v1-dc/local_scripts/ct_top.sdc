...
set CPU_PERIOD			[expr (1000.00/1300)] ;# CPU is constrained at 1300MHz

if {$IF_READ_BUIDIN_VARIABLES} {
   set SETUP_UNCERTAINTY	0.30
   set HOLD_UNCERTAINTY		0.03
   set MAX_FANOUT		32
   set MAX_TRANSITION		0.1

   set DRIVING_CELL             "BUFFD2BWP12T40P140"
   set LOAD_PIN			"AN2D2BWP12T40P140/A1"
}
...
if {$parent_path==""} {
   ...
   group_path -name INTERNAL_REG2REG -from [all_registers] -to [all_registers] -weight 5
   group_path -name IO_IN2REG -from $CPU_CLK_INS -to [all_registers] -weight 2
   group_path -name IO_REG2OUT -from [all_registers] -to $CPU_CLK_OUTS -weight 2
   group_path -name IO_IN2OUT -from $CPU_CLK_INS -to $CPU_CLK_OUTS -weight 1
