...
3. **RISC-V 地址对齐**：指令至少 2 字节对齐，PC 最低位恒为 0（不存入 tag），实际有效比较位是 15 位，覆盖 32KB 地址范围内的唯一性。配合 16 项的小容量，误命中率在实际工作负载中极低。
...
即高位（bit 38:20）直接沿用当前取指 PC 的高位。这个设计成立的前提是：**分支目标与分支指令的高位地址相同**，即跳转目标在当前 1MB（2^20 = 1MB）对齐区域内（对应 bit 19 是分支/非分支的分界）。

对于大多数用户态代码和操作系统内核来说，函数调用和循环跳转的目标地址通常在很近的地方，1MB 范围内跳转涵盖绝大多数情况。若跳转超出 1MB 范围（如跨段 call），这种情况下 L0 BTB 会在 IP 级被检测到预测错误，由 addrgen 修正——L0 BTB 只需处理最常见情况，边缘情况容错处理。
...
| 完整目标 vs 面积 | 只存 20 位 target | 完整 39 位跳转地址 |
