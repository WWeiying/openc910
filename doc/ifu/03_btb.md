...
BTB 本质上是一个小型的**直接映射或组相联缓存**，以分支指令的 PC（vpc）为 key，存储其跳转目标地址。每次 PCGEN 送出新的 PC 时，BTB 同步查找，如果命中就立即把预测的目标地址用于下一拍的取指。
...
│                                                                 │
│  ┌──────────────┐   最快，0 延迟，容量最小                        │
│  │   L0 BTB     │   全相联，16 项，PCGEN 级直接命中直接跳           │
│  │  (l0_btb.v)  │   存储完整目标 PC + icache way 预测              │
│  └──────────────┘                                               │
│         │ miss                                                  │
│         ▼                                                       │
│  ┌──────────────┐   1 拍延迟，容量中等                            │
│  │     BTB      │   4 路组相联，1024 项（实际 512×2 bank）         │
...
**关键分工说明**：
- **L0 BTB** 是 BTB 的"热点缓存"：容量极小但速度最快，命中即在 PCGEN 级决策，无延迟。
...
注释中提到了 BTB_1024 和 BTB_512 两种配置。当前实例化的是 `ct_spsram_512x22`，地址线 `btb_index[8:0]`（9 位，寻址 512 行）。两个 bank 各 512 行 × 22 位，同一逻辑 index[9:0] 的 bit[9] 选择 bank，bit[8:0] 寻址行内偏移——这样等效于 1024 行的逻辑深度，通过两个物理 512 行 SRAM 拼成。
...
```
  VPC（40位）:  [39:20]高位不变  |  [19:0] 低 20 位变化
               ─────────────────────────────────────────
  Target PC:   [39:20]同分支PC   |  [19:0] 存入 BTB Data
```

BTB 的 Tag Array 存储 tag = `{vpc[19:13], vpc[2:0]}`（10 位），查找时用 index 定位行、tag 匹配确认。当命中时，上游（ifdp 或 ifctrl）将 BTB 读出的 target[19:0] 与**分支指令 VPC 的高 20 位**（`vpc[39:20]`）拼接，恢复完整的 40 位目标地址。

这样做的代价是：若分支目标的高 20 位与分支指令不同（极少见的"大跨度跳转"或间接跳转），BTB 会给出错误预测，由后级（IP 级）检测并纠正。这类情况由 **Indirect BTB** 负责兜底。

---

## 3. Index 与 Tag 编码

### 3.1 index = vpc[12:3]：为什么是这段？

```verilog
// 来自 pcgen 或 addrgen 计算后的 index，例如：
// pcgen_btb_index[9:0] = vpc[12:3]
```

**10 位 index** 对应 1024 个逻辑条目（实际映射到两个 512 行 bank）。

- **为什么从 bit3 开始**：RISC-V 指令最小 16 位（2 字节），意味着 VPC 的 bit0 恒为 0。C910 每次取一个 cacheline（64 字节），以 cacheline 为单位做 BTB 查找，因此 bit[2:0]（字节偏移，标识 cacheline 内的哪条指令）**不参与 index**，而是留给 tag 使用。
- **为什么到 bit12**：bit[12:3] 共 10 位，对应 1K 个 cacheline。这平衡了容量与硬件面积：index 太少会导致严重的组冲突（aliasing），index 太多则 SRAM 深度增大，面积功耗上升。
- **直觉理解**：VPC[12:3] 是"页内的 cacheline 编号"。一个 4KB 页有 64 条 cacheline，10 位可区分 1K 条，跨越 4 个 4KB 页的范围。

### 3.2 tag = {vpc[19:13], vpc[2:0]}：为什么跳过 [12:3]？低 3 位的作用？

```
VPC（以 IF 级取指为例）：
  bit：  39..20    19..13    12..3     2..0
         高位      ─────────────────────────
         (不存)     tag[9:7]  (=index)  tag[2:0]
```

Tag 的构成逻辑：

1. **跳过 vpc[12:3]**：这段已经作为 index 使用了，不需要再存入 tag（否则是冗余信息，浪费存储）。
2. **vpc[19:13]（7 位）**：这段信息不在 index 中，可以区分**相同 index 但不同"大范围地址"**的指令，是主要的歧义消解手段。
3. **vpc[2:0]（3 位）**：这是 cacheline 内的**指令偏移**，标识分支指令在取指包内的位置（第几条）。BTB 是以 cacheline 为粒度查找的，但一个 cacheline 内可能有多条分支指令，低 3 位用来精确区分"同一 cacheline 内哪条指令是分支"。

```
举例：
  地址 0x1000_0008（bit[2:0]=0b100）和
  地址 0x1000_0010（bit[2:0]=0b000）
  两者 vpc[12:3] 相同（都在同一个 cacheline 对齐的范围），
  但 vpc[2:0] 不同，因此 tag 不同，BTB 能正确区分。
```
...
BTB 写入时并不同时写全部 4 路——而是每次只写 **2 路**（bank0 中的 1 路 + bank1 中的 1 路）。具体选哪路，由 `refill_buf_wen` 决定：
...
`refill_buf_tag[2:1]` 是 tag 的第 1、2 位，即 `vpc[4:3]`（cacheline 内更细粒度的地址位）。这 2 位共 4 种取值，决定写哪路：

| `tag[2:1]`（= `vpc[4:3]`） | wen[3:0] | 写入路 |
|---------------------------|----------|--------|
| 2'b00 | 4'b1110 | Way0 |
| 2'b01 | 4'b1101 | Way1 |
| 2'b10 | 4'b1011 | Way2 |
...
这是一种**伪随机替换**策略。同一个 index 位置的 4 路对应同一组的 4 个"slot"。当同一个 index 需要存放不同地址的分支时，以地址本身的位决定写哪路，避免了需要额外 FIFO 指针或 LRU 状态寄存器，实现简单且硬件开销小。
...
注意这里的 pc[3]、pc[2] 对应 vpc[4]、vpc[3]（因为 vpc[0] 是字节对齐位）。`{!(pc[3]^pc[2]), pc[3]^pc[2]}` 给出 2 位互斥编码，相当于 XOR 决定写"奇路"还是"偶路"。完整的 4 路版本就是对 `tag[2:1]` 做 4 选 1 译码。
...
注意 tag 数据和 data 数据都**同时写入两个 bank**，但通过 `wen` 只激活对应的那一路：
...
```
物理 SRAM：2 个 bank，每 bank 512 行
逻辑 index：10 位，对应 1024 个逻辑行
每次 INV 写入：wen=4'b0000，4 路全写 → 覆盖 1 个 index 的全部 4 路

但每次 index 由 btb_inval_cnt 给出：
  - btb_inval_cnt 从 511 递减到 0 → 512 次
  - btb_index[9:0] = btb_inval_cnt[9:0]
  - 实际地址线进入 SRAM 是 btb_index[8:0]（9位）

因此：btb_inval_cnt[9:0] 从 511 到 0：
  btb_inval_cnt[9]=0 → bank0 的 index[8:0] 从 511 到 0  （512 行）
  btb_inval_cnt[9]=1 → bank1 的 index[8:0] 从 511 到 0  （512 行）

Wait：INV_CNT_VAL = 10'b0111111111 = 511，从 511 倒数到 0 共 512 步。
btb_index[9] 作为 bank 选择位：
  当 cnt 从 511→256：bit9=0，写 bank0 的 0..255 行
  当 cnt 从 255→0：  bit9=0，也写 bank0 的 0..255 行？

重新审视：cnt=511 时 bit9=0，index[8:0]=255；cnt=256时bit9=0，index[8:0]=0
实际上 cnt[9] 恒为 0（因为从 0111_1111_11 = 511 开始，bit9 就是 0）。

真正的解释：每次 INV 写一个 index（对应 SRAM 里的 1 行），wen[3:0]=4'b0000
对 4 路全部写 valid=0。512 个 index（0..511）覆盖整个 512 行 SRAM，
两个 bank 共享同一个 cen_b 和 index，因此同时被清零。
→ 512 次 INV，每次清 1 行（2 个 bank 各 1 行），共清 512×2=1024 路次，
   对应 1024 个逻辑条目的全部 valid 位 = 清空整个 BTB。
```
